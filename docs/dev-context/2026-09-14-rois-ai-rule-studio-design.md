...
- 时间：2026-09-14 09:17:29 PDT
- Wing：`rois-ai`
- Topic：`rule-studio-design`
- Title：ROIS Rule Studio design alternatives
- Git branch：`main`

## 本轮对话上下文

Design-only task: user requested AI-powered rule authoring requirements, competitor context and three unique HTML mocks, explicitly no application implementation. Created docs/superpowers/specs/2026-09-14-rule-studio-Ver1/ with index, Workbench, Dossier, Rule Lab, requirements, inspected source inventory and verification receipt. Recommended name ROIS Rule Studio; recommend A shell with B evidence and C visual examples. These are proposals, not approved business decisions.
Seven steps: source/match, clarify, define, test specification, build candidate, individual then optimizer validation, review package. Step-local chat, immutable revisions and downstream evidence invalidation. Rule-only scope enforced by broker/isolated builds/diff allowlist, not prompt alone. Worked rule X rolling home-base calendar days / Y separate or consecutive days off; blank completeness, group snapshot, overlapping duty, DST and boundary context require explicit contract. Partial optimizer candidates must not misuse final blank-day legality.
Read-only delegated inventory reviewed by primary: 7505 minimum off-day count and 7508 rolling free-day behavior are partial candidates; 7305 max consecutive work is not minimum consecutive off. Rust-side PyO3 dispatch present, pbs-engine absent. No solver performance or runtime functionality certified. Rave guide is independent educational material, not official competitive proof.
Playwright prototype verification PASS for all three designs, including fail/fix/rerun and stale locks. Exact commands and screenshot list in package README.md. All latest screenshots visually inspected; docs/assets/screenshots/rule-studio/ preserves earlier versions. check:ui PASS 0 hard/124 existing warnings. Documentation links/assets/source paths/whitespace PASS. No application source, DB, commits, pushes or deployments. Existing unrelated worktree modifications preserved. Preserve the prior LATEST.md contents because they belong to concurrent work; this named context file is the durable handoff.
...
1. `NEXT_CONTEXT.md`
2. 本文件：`docs/dev-context/2026-09-14-rois-ai-rule-studio-design.md`
