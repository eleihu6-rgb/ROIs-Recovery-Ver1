...
- 时间：2026-04-30 10:45:01 CST
- Wing：`pbs`
- Topic：`pbs-bidding-calendar-aa-data-sources-implemented`
- Title：pbs-bidding-calendar-aa-data-sources-implemented
- Git branch：`main`

## 本轮对话上下文

本轮继续 PBS AA 左侧 BIDDING CALENDAR 数据源补齐实现。

用户已确认 spec：docs/superpowers/specs/2026-04-30-pbs-bidding-calendar-aa-data-sources-design.md。
本轮将状态更新为“已确认，实施中”。

实现内容：
- 新增 contract：packages/contracts/pbs-bidding-calendar.js / .d.ts。
- 新增后端只读接口 GET /api/bidding-calendar/current：pbs-server/src/routes/bidding-calendar.ts。
- 新增后端聚合 service：pbs-server/src/services/calendar/bidding-calendar-service.ts。
- app.ts 注册 biddingCalendarService，支持测试注入；skipDatabase 下提供占位 mock。
- service 聚合：current period、existing bid、pbs_bid_day_off -> day_off_bid、pbs_bid_group property_code=128 -> live pairing -> pairing_bid、weekend computed event、planned absence 权限不可用 warning 降级。
- 保留 /api/calendar-days-off/current 作为 Days Off 编辑接口。
- 前端新增 biddingCalendarService/useBiddingCalendar/bidding-calendar-mappers。
- DashboardSchedulePanel 默认读取综合日历；Days Off 路由才加载 calendar-days-off draft 用作可编辑 DO overlay；保存 DO 后 invalidate calendar-days-off、bidding-calendar、tierPageData。
- ScheduleCalendarCell 新增 weekend 标记，日历 cell 用浅背景显示 weekend，不生成抢占事件条。
- 测试更新：新增 backend route test、frontend mapper test，并调整 shared layout / app routes / pairing dashboard tests 使用 biddingCalendarService。

验证：
- pbs-server npm test：118 passed。
- pbs-server npm run build：通过。
- pbs-portal npm test：196 passed。
- pbs-portal npm run lint：通过。
- pbs-portal npm run build：通过。
- 根目录 npm run verify:pbs：通过。
- 真实接口验证：
  - http://localhost:3002/api/bidding-calendar/current 200。
  - http://localhost:3030/api/bidding-calendar/current 200。
  - 使用 crew employeeNo=3002 返回 dayOff=29、pairing=0、weekend=8、planned absence warning。
  - pairing=0 是当前库没有 property_code=128 specific Pairing ID bid 数据，不是接口失败。

注意事项：
- planned absence 第一阶段只做权限探测和 warning 降级，不造假数据。
- 后续若要完整展示 planned absence，需要确认权威 live 表和授权。
...
1. `NEXT_CONTEXT.md`
2. 本文件：`docs/dev-context/2026-04-30-pbs-pbs-bidding-calendar-aa-data-sources-implemented.md`
