...
- 时间：2026-04-30 10:12:11 CST
- Wing：`pbs`
- Topic：`pbs-tier-db-migration-applied`
- Title：PBS Tier 数据库迁移已应用
- Git branch：`main`

## 本轮对话上下文

本轮在用户确认后，针对 PBS Tier 术语迁移后的 500 问题实际执行数据库迁移。

问题现象：
- 用户访问 `http://localhost:3030/api/calendar-days-off/current` 已登录后出现 500。
- 3030 是 pbs-portal Vite dev server，`/api` 代理到 pbs-server 3002。

确认原因：
- `pbs-server/.env` 当前连接到 rois 库的 `f8_pbs` schema。
- 代码已迁到新结构：`pbs_bid_tier`、`tier`、`total_tiers`、`max_tiers` 等。
- 数据库迁移前仍是旧结构：`pbs_bid_layer`、`pbs_bid_day_off.layer`、`pbs_bid.total_layers`、`pbs_period.max_layers`、award `*_layer`。

执行过程：
- 第一次执行 `sql/migration/2026-04-30-rename-pbs-layer-to-tier.sql` 时失败并回滚，原因是 `f8_pbs` schema 没有 `dictionary` 表，而 migration 里直接迁移 `PBS_MAX_LAYERS -> PBS_MAX_TIERS` 参数。
- 已修正 migration：把 dictionary 参数迁移包进 `if to_regclass('dictionary') is not null then ... end if;`，使 PBS 独立 schema 没有 dictionary 时也能完成业务表结构 rename。
- 第二次执行 migration 成功，目标为 `database_name=rois`、`current_schema=f8_pbs`。

迁移后验证：
- 当前 schema 已有 `pbs_bid_tier`，没有 `pbs_bid_layer`。
- 关键列已迁为：`pbs_bid_tier.tier`、`pbs_bid_day_off.tier/tier_id`、`pbs_bid_group.tier_id`、`pbs_bid_error.tier`、`pbs_bid.total_tiers`、`pbs_period.max_tiers`、`pbs_award_result.awarded_tier`、`pbs_award_item.matched_tier`。
- 用临时 JWT 分别请求：
  - `http://localhost:3002/api/calendar-days-off/current` 返回 200。
  - `http://localhost:3030/api/calendar-days-off/current` 返回 200。
- 返回体示例使用新 contract：`data.draft.tiers`。

注意：
- 本轮没有提交 git。
- 不要把 migration 中 `dictionary` guard 去掉；PBS 独立 schema 可能没有基础 dictionary 表。
...
1. `NEXT_CONTEXT.md`
2. 本文件：`docs/dev-context/2026-04-30-pbs-pbs-tier-db-migration-applied.md`
