...
- 时间：2026-09-14 15:47:07 PDT
- Wing：`gantt`
- Topic：`case4-shared-cost-tier`
- Title：case4-shared-cost-tier
- Git branch：`main`

## 本轮对话上下文

User found Case4 missing left By cost tier. Fixed by injecting same existing PlanTree used in Cases1–3 into OpenRecoveryWorkspace after pairing creation. PlanTree accepts optional explicit groups; default legacy plans behavior preserved. Best-group selection logic shared, not copied. Shared tier leaves get stable test IDs. Pairing Options gate before saved pairing unchanged.
Cost-tier semantics method minimum priced executable cost, not per-candidate filtering. Standby USD0 leaf retains all7 candidates incl USD326.25; availableUSD410/moveupUSD2470 in0–10k. Unpriced/failed/pending excluded, mixed currencies not compared. Legacy unrelated summary fallback not changed.
32 focused tests PASS, Case4 tier->each of3 methods plus cost/detail/preview UI PASS,3 legacy UI tests PASS, Help UI PASS including new screenshot. UI gate0hard/124warnings and diff PASS. Tsc only3 existing service-status-pill errors. Screenshot case4-by-cost-tier-Ver1.png inspected plus legacy latest and HelpVer8.
Help/skill145 and shared design updated. Report docs/test-cases/crew-recovery/2026-09-14-1547-case4-cost-tier-Ver1.md. No DB writes, Apply/Save/reset. Preserve user pairing152689 and real-roster GH pool enhancement. Supporting read-only audit delegated and reviewed. No commit/push.
...
1. `NEXT_CONTEXT.md`
2. 本文件：`docs/dev-context/2026-09-14-gantt-case4-shared-cost-tier.md`
