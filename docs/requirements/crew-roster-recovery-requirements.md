...
- Standby Crew 替换方案。
- Crew 之间交换任务方案。
- Cross-base Recovery（7.3.1）：跨基地 Standby、跨基地 Swap、跨基地 Destination 三类子选项；与基础方案（方案一/方案二）并列筛选，不归入“方案一/方案二”编号。
- 方案前后差异、成本和影响数量展示。
- 方案确认、保存、生效和再次检查。
...
机组恢复能力统一通过 Roster Pane 的 `Alert Center` 告警入口和 Live Gantt Roster 告警浮显进入；`System → Interface` 当前仅进入 OPS Flight Schedule Simulation（5.1），不作为 Recovery 入口；独立顶层 `Recovery` 菜单已废弃（4.2 / 14.6）。
...
- 直接转移、双向交换和 Callout Standby 均必须检查恢复后目标 Crew 的完整 Roster 规则结果。Live 8004 适配器必须同时提供 `pairing_segment.fleet_seg`（含既有回退链）与 `crew_fleet.{fleet_specific, ac_type, fleet_grp}` 有效期资格输入；详见 15.8。
...
方案类型选择器必须使用整块可点击的方案卡，而不是仅依赖方案名称文字作为点击入口。此处"方案卡"即 14.4 中"方法选择区域"，二者为同一 UI 元素：承担按方案类型（方案一/方案二/Cross-base 子选项）过滤候选的职责，候选区单独展示具体 Crew 恢复选项，二者职责不重叠。每张卡应展示方案名称、方案说明、候选数量、可执行数量和代表性成本，并通过方案专属色带、边框、底色以及明确的 `Selected` 状态标识当前方案。点击卡片任意主要区域都应切换方案，并立即刷新下方候选列表；切换过程中应显示稳定的选中反馈，不得出现点击命中区域过小或点击后无可见变化的情况。
...
7.7 节内 Detail 内的“在 Live Gantt 中预览”按钮和方案列表/方案卡的 Preview 按钮入口等价：两者均调用同一会话级 Preview 渲染器，共享 Crew 置顶、双态叠加、Before/After 颜色与 Pairing 同行的语义；不得在两条路径上分别维护两套 Preview 状态。Detail 入口还附带明细表格的当前选项上下文。
...
当前 Recovery 已通过 Alert Center 工作流接入：候选生成严格使用当前已加载的 Live 数据，前端支持完整 Roster 转移、双向交换、Standby Callout、方案级比较、Detail 表格和 Live Gantt 前后预览。本期 `Apply` 的真实路径由 `recovery-violation-dialog.tsx` 的 `apply()` 构造 `RecoveryDraftPlan` 并调用 `addDraftOp`，将恢复方案写入当前 Live Gantt 的未保存 Draft；正式落库复用 Gantt 既有 `Save`/`Ctrl+S` 链路（锁、Rule 预检、事务提交、缓存刷新），不再由前端逐行直接写入 `roster_flight`。`POST /api/recovery/apply-option`、`POST /api/recovery/methods/{roster-assignment,callout-standby,cross-base}` 等后端接口作为面向后续 Rule 和独立后端执行链路的前向兼容入口存在于 `live-server/src/routes/recovery/recovery.ts`，本期前端 `Apply` 不调用这些接口；旧的 `simulate-flight-change` 演示接口和内存 Session 接口仅保留在 OPS 模拟页面，不代表 Live Recovery 的正式执行链路。
...
- 系统自动生成方案一（直接转移/双向交换）和方案二（当前基地 Standby Crew 替换）两类基础方案，并在每个方案下列出不同候选 Crew 对应的恢复选项。Cross-base Recovery（7.3.1）作为本期纳入的扩展恢复能力独立展示于方案比较区，其三类子选项（Callout Standby / Swap / Destination）与基础方案并列筛选；不归入“方案一/方案二”编号。
...
- Live 8004 数据源必须同时提供已分配航段的机型数据（优先 `pairing_segment.fleet_seg`，再按既有回退链读取航班/Pairing 机型）和 Crew 有效期机型资格数据（`crew_fleet.fleet_specific`、`ac_type`、`fleet_grp`）；缺少任一适配器不得以“重算成功”代替真实检查。当前 Live schema 的 `crew_fleet` 没有 `is_valid` 字段，资格有效性统一由 `eff_dt`、`exp_dt` 和 Rule 计算逻辑判断。
...
1. **（编号占位/历史首条）** 当前文档不保留旧版本 Q1 文本；如需追溯，参考版本记录 0.1 / 0.2 / 0.3 中曾列出的初始待确认项。本节自 Q2 起为当前有效待确认事项。
...
| 1.4 | 2026-09-05 | 根据代码开发复核新增实现边界和待澄清项，补充 Violation 数据完整性、Roster 身份、服务端校验、异步结果、时间/飞时口径、旧 Demo 链路及 Redis 运行前提（注：v1.4 是 v1.0 的实现复核而非替代，v1.0 确认的 CC/FD Manday `blh` 口径为业务基线，v1.4 揭示的 `ybh` 偏差属于待解决的实现 bug） |
...
| 3.6 | 2026-09-07 | 修复本地开发环境反复出现的告警缺失：`SKIP_RUST_BINS=true` 时跳过仅用于构建 7500 状态的 `check-7500-ref` 预处理，并将未指定规则范围的 Live 冷启动限定为继续执行可用的 8004 JavaScript 检查；避免每次服务重启因缺少 7500 二进制提前失败、Alert Center 只显示旧告警。显式指定 `--rules` 的行为不改变，生产环境仍必须提供完整 Rust 二进制。 |
| 3.7 | 2026-09-07 | 修复 Live 8004 适配器缺失：补充按航段读取 `pairing_segment.fleet_seg`/航班机型，以及读取 `crew_fleet` 的 `fleet_specific`、`ac_type`、`fleet_grp` 有效期资格；修复多表查询 `is_deleted` 歧义；增加 Live 适配器 SQL 回归测试。真实重算已验证 Crew `1012` / Pairing `135559` 生成 `8004` Fleet 告警。 |
| 3.8 | 2026-09-07 | 重构需求文档一致性：4.1 本期范围补充 Cross-base Recovery（跨基地 Standby/Swap/Destination 三类子选项，与基础方案并列筛选）；编号修订（`## 4.3`→`### 4.3`、`#### 6.3.1`→`#### 6.4.1`、`#### 6.4.1`→`#### 6.4.2`、`### 17.1~17.7`→`### 9.2~9.8`、header 版本 `2.5`→`3.7`）；语义修订（13 章 Apply 路径由后端 `apply-option` 改为前端 `apply()` 写 Draft + Gantt Save 落库；4.1 之后入口路径明确为 `Alert Center` + Roster 浮显，`System → Interface` 仅进入 OPS 模拟；14.4 14.4 增补 Cross-base 与基础方案并列筛选关系；7.1 增补 `pairing_segment.fleet_seg`/`crew_fleet` 适配器交叉引用至 15.8；7.7 增补 Detail 内"在 Live Gantt 中预览"与方案卡 `Preview` 入口等价说明；7.4 增补"方案卡"与 14.4"方法选择区域"为同一 UI 元素说明）；追溯修复（16.2 增加 Q1 编号占位；版本表 v1.4 标注为 v1.0 实现复核）。仅修改本文件，未触及目录内其他文档。 |
