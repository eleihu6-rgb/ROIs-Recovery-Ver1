...
├── build.sh             # Linux 编译/打包脚本（./build.sh / ./build.sh pack）
├── scripts/             # 辅助脚本目录
│   ├── build.bat        # Windows 编译脚本
│   ├── deploy.sh        # 客户机运行时管理（install/start/stop/restart/status）
│   ├── start.sh         # Linux 源码启动脚本（开发用）
│   └── start.bat        # Windows 源码启动脚本（开发用）
├── optimize_server.spec # PyInstaller 配置文件
