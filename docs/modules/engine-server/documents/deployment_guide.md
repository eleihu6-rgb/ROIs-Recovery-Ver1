...
在Windows平台上，使用 `scripts\start.bat` 脚本启动服务：
...
在Linux平台上，使用 `scripts/start.sh` 脚本启动服务：
...
Windows 平台的打包目前仍走 `scripts\build.bat` 单步流程：构建产物 `dist/optimize_server.exe`，
直接双击或 `optimize_server.exe` 运行；目前 `./build.sh pack` 的发布包流程仅覆盖 Linux。
...
1. 创建独立的 `build-venv/` → 安装 `requirements.txt` 与 `pyinstaller`
   → 生成 `git.properties` → 执行 `pyinstaller optimize_server.spec`
   → 产物为 `dist/optimize_server`（onefile，约 17 MB）
2. 把 `dist/optimize_server` + `config.yaml.example` + `scripts/deploy.sh`（运行时脚本）
   打成 `dist/rois-optimizer-server.tar.gz`
...
> Windows 暂未集成 `./build.sh pack` 一键流程，需手动执行上述步骤。
...
完成后 `dist/rois-optimizer-server.tar.gz` 会带着低版本 glibc 兼容的二进制。
...
```bash
# 测试API接口
curl -X GET "http://localhost:3003/api/system/info" -H "Authorization: Bearer your_secret_token"
```
