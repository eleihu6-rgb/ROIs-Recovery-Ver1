...
- Production crew app traffic uses public HTTPS CMS API host `https://cr.rois.one`; localhost is only a simulator/development fallback.
...
This endpoint is intended for crew app access from the public internet at `https://cr.rois.one/api/mobile-roster/session`. Add exact path `/api/mobile-roster/session` to `PUBLIC_PATHS` in `live-server/src/plugins/auth.ts`, but keep credential verification inside the route before any roster data access. Do not touch Cloudflare/DNS/tunnel config in this task; routing `cr.rois.one` is a separate deployment validation step.
...
- Add live-server API base resolver, or generalize `resolveEkRosterApiBaseUrl`, so F8 uses configured production HTTPS base URL such as `https://cr.rois.one/api` and defaults to `http://127.0.0.1:3000/api` only in development.
...
For public-device validation, route `https://cr.rois.one/api/mobile-roster/session` to this live-server origin through the approved Cloudflare/deployment path and verify with a non-secret HTTP smoke. Do not change routing as part of this implementation plan unless Ryan explicitly asks for deployment work.
