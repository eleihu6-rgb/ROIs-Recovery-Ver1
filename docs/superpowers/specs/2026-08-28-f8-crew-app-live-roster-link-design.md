...
- Public API host: production app traffic should use an HTTPS public CMS API host such as `https://cr.rois.one`
...
`POST https://cr.rois.one/api/mobile-roster/session`
...
- The crew app connects from the public internet, so production exposure must use HTTPS, a CORS allowlist for approved app/web origins, low-cardinality request logging, and a real mobile auth mechanism. Localhost is only a simulator/development fallback.
...
- Configure F8 API base URL for production as `https://cr.rois.one/api`, with `http://127.0.0.1:3000/api` only as the local simulator fallback.
...
- `cr.rois.one` routing/tunnel/DNS is a deployment concern and must be validated separately before public simulator/device testing.
