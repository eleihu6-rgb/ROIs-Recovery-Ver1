Release note automation for the ROIS-AI gantt app in /home/yuan.z/rois/rois-ai.
This job runs unattended every Wednesday and Friday at 03:00 UTC (11:00 Beijing).
It authors the Release note ONLY — Help refresh is a separate daily job.
...
2. Verify — ALL must pass before commit:
...
3. Commit + push + deploy:
   - Stage ONLY the files you changed (release-data.ts, version.ts, and any e2e
     spec you updated). Do NOT stage unrelated pre-existing working-tree files.
   - Commit message: `docs(gantt): Release N notes` + short body + trailer
     `Co-Authored-By: Claude <noreply@anthropic.com>`.
   - `git push origin main`.
   - `~/rois/rois.sh build gantt` to deploy the frontend to UAT.

4. Report a concise final summary: what changed, test PASS/FAIL counts, commit
