...
import { buildTimeCompareClause } from "./pairing-search-condition-shared.js";
...
const buildPairingCheckEventLocalExpression = ({
  ...
  const timestampColumn =
    timeType === "check_out" ? "debrief_end_utc" : "brief_start_utc";
...
export const buildPairingCheckTimeApplicabilityCondition = (
  ...
  const factColumn =
    property.bid.timeType === "check_out"
      ? "check_out_local"
      : "check_in_local";
  const timestampColumn =
    property.bid.timeType === "check_out"
      ? "debrief_end_utc"
      : "brief_start_utc";
  const timeExpression = context.useCurrentRulesFacts
    ? `facts.${factColumn}::time`
    : buildPairingCheckEventLocalExpression({
        liveSchema,
        part: "time",
        timeType: property.bid.timeType,
      });
  const dateExpression = context.useCurrentRulesFacts
    ? `facts.${factColumn}::date`
    : buildPairingCheckEventLocalExpression({
        liveSchema,
        part: "date",
        timeType: property.bid.timeType,
      });
  const factExistsClause = context.useCurrentRulesFacts
    ? `${timeExpression} is not null`
    : `exists (
      select 1
      from ${liveSchema}.pairing_segment applicability_segment
      where applicability_segment.pairing_id = p.id
        and applicability_segment.is_deleted = 0
        and applicability_segment.${timestampColumn} is not null
    )`;
  const clauses = [factExistsClause];
  ...
  if (property.bid.dateScope.mode === "specific_dates") {
    clauses.push(
      `${dateExpression} = any(${sqlBuilder.addParam(property.bid.dateScope.dates)}::date[])`,
    );
  } else {
    clauses.push(
      `${dateExpression} between ${sqlBuilder.addParam(property.bid.dateScope.from)}::date and ${sqlBuilder.addParam(property.bid.dateScope.to)}::date`,
    );
  }
...
const buildDutyOnTimeExpression = (liveSchema: string) => `
  (
    select (min(coalesce(s.duty_sch_str_dt_utc, s.brief_start_utc, s.sch_str_dt_utc)) at time zone 'UTC')::time
    from ${liveSchema}.pairing_segment s
    where s.pairing_id = p.id
      and s.is_deleted = 0
  )
...
const buildEnrouteTimeExistsClause = (
  ...
  if (quantifier === "every") {
    const compareClause = buildTimeCompareClause(
      sqlBuilder,
      timeExpression,
      bid,
    );
...
export const buildTimePreviewCondition = (
  ...
  switch (property.propertyCode) {
    case 103: {
      ...
      const timeClause = buildTimeCompareClause(
        sqlBuilder,
        timeExpression,
        timeBid,
      );
      ...
      if (property.bid.dateScope.mode === "specific_dates") {
        ...
        if (
          ...
          throw new LineholderBidServiceError(
            400,
            `Invalid event dates for ${property.name}.`,
          );
        }
      ...
      if (
        ...
        property.bid.dateScope.to < property.bid.dateScope.from ||
        !isDateInPairingSearchPeriod(property.bid.dateScope.from, context) ||
        !isDateInPairingSearchPeriod(property.bid.dateScope.to, context)
      ) {
        throw new LineholderBidServiceError(
          400,
          `Invalid event date range for ${property.name}.`,
        );
      }

      const fromPlaceholder = sqlBuilder.addParam(property.bid.dateScope.from);
      const toPlaceholder = sqlBuilder.addParam(property.bid.dateScope.to);
      return `(${dateExpression} between ${fromPlaceholder}::date and ${toPlaceholder}::date and ${timeClause})`;
    }
    case 134: {
    ...
    case 139: {
      if (
        property.bid.type !== "time-date" &&
        property.bid.type !== "time-range-date"
      ) {
        break;
      }
    ...
    case 135: {
      ...
      return buildTimeCompareClause(
        sqlBuilder,
        buildReleaseTimeExpression(liveSchema),
        property.bid,
      );
    }
    case 140: {
    ...
    case 114: {
      ...
      return buildEnrouteTimeExistsClause(
        sqlBuilder,
        liveSchema,
        "brief_start_utc",
        property.bid,
    ...
    case 120: {
      ...
      return buildTimeCompareClause(
        sqlBuilder,
        buildDutyOnTimeExpression(liveSchema),
        property.bid,
      );
    }
    case 164: {
      ...
      return buildTimeCompareClause(
        sqlBuilder,
        buildDepartureTimeExpression(liveSchema),
        property.bid,
      );
    }
    case 136: {
      ...
      const compareBid = {
        ...property.bid,
        operator:
          property.bid.operator === "<" || property.bid.operator === ">"
