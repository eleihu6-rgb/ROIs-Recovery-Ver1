...
const HANDLER_FILES: Record<GeneratedSqlPreflightHandler, string> = {
  special: "src/services/pairing-search/pairing-search-condition-builder.ts",
...
const extractDispatcherCodes = (handler: GeneratedSqlPreflightHandler): number[] => {
  ...
  const visit = (node: ts.Node): void => {
    if (
      ...
      && node.left.getText(sourceFile) === "property.propertyCode"
      && ts.isNumericLiteral(node.right)
    ) {
      codes.add(Number.parseInt(node.right.text, 10));
    }
    if (
...
export const runGeneratedSqlStructureAudit = (): void => {
  ...
  for (const preflightCase of generatedSqlPreflightCases) {
    ...
    const completeSql = `${sqlBuilder.renderCtes()}\n${condition}`;
    if (!condition.trim()) {
    ...
    const placeholders = [...completeSql.matchAll(/\$(\d+)/g)].map((match) => Number(match[1]));
