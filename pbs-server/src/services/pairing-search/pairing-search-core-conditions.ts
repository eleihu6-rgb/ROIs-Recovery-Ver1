...
import {
  isStablePairingId,
  normalizePairingBidIds,
} from "./pairing-id-utils.js";
import {
...
const buildPairingLengthPreferenceClause = (
  ...
  if (bid.minDays == null && bid.maxDays == null) {
    throw new LineholderBidServiceError(
      400,
      `${propertyName} requires minimum days, maximum days, or both.`,
    );
  }

  if (
    bid.minDays != null &&
    (!Number.isSafeInteger(bid.minDays) || bid.minDays < 1)
  ) {
    throw new LineholderBidServiceError(
      400,
      `Invalid pairing length for ${propertyName}.`,
    );
  }

  if (
    bid.maxDays != null &&
    (!Number.isSafeInteger(bid.maxDays) || bid.maxDays < 1)
  ) {
    throw new LineholderBidServiceError(
      400,
      `Invalid pairing length for ${propertyName}.`,
  ...
  if (bid.minDays != null && bid.maxDays != null) {
    clauses.push(
      `p.tafb between ${sqlBuilder.addParam(bid.minDays)} and ${sqlBuilder.addParam(bid.maxDays)}`,
    );
  ...
  if (bid.dateScope) {
    const occurrenceStartUtcExpression =
      buildOccurrenceStartUtcExpression(liveSchema);
    const dateExpression = `(${occurrenceStartUtcExpression} at time zone 'UTC')::date`;
    ...
    if (bid.dateScope.mode === "specific_dates") {
      ...
      if (
        ...
        throw new LineholderBidServiceError(
          400,
          `Invalid pairing start dates for ${propertyName}.`,
        );
      }

      clauses.push(
        `${dateExpression} = any(${sqlBuilder.addParam(dates)}::date[])`,
      );
    } else {
      if (
        ...
        throw new LineholderBidServiceError(
          400,
          `Invalid pairing start date range for ${propertyName}.`,
        );
      }

      clauses.push(
        `${dateExpression} between ${sqlBuilder.addParam(bid.dateScope.from)}::date and ${sqlBuilder.addParam(bid.dateScope.to)}::date`,
      );
    }
  }
...
const buildMonthEndCarryoverClause = (
  ...
  if (bid.operator === "Between") {
    if (
      !isPositiveInteger(bid.from) ||
      !isPositiveInteger(bid.to) ||
      (typeof bid.from === "number" &&
        typeof bid.to === "number" &&
        bid.from > bid.to)
    ) {
      throw new LineholderBidServiceError(
        400,
        `Invalid carry-out days for ${propertyName}.`,
      );
    }
  ...
  switch (property.propertyCode) {
    case 102: {
      if (property.bid.type !== "pairing-preference") {
        throw new LineholderBidServiceError(
          400,
          `${property.name} must use Pairing IDs selected from the list.`,
        );
      }

      const normalizedIds = normalizePairingBidIds(property.bid.pairingIds);
      if (
        normalizedIds.length === 0 ||
        normalizedIds.some((id) => !isStablePairingId(id))
      ) {
        throw new LineholderBidServiceError(
          400,
          `${property.name} requires Pairing IDs selected from the list.`,
        );
      }

      return `p.id = any(${sqlBuilder.addParam(normalizedIds)}::bigint[])`;
    }
    case 112:
      if (property.bid.type === "pairing-length-preference") {
        return buildPairingLengthPreferenceClause(
          property.bid,
          liveSchema,
          sqlBuilder,
          property.name,
    ...
    case 105: {
      ...
      return buildDurationCompareClause(
        sqlBuilder,
    ...
    case 106: {
      ...
      if (property.bid.type === "date-or-dow-list") {
        ...
        if (property.bid.dates.length > 0) {
          clauses.push(
            `${departingDateExpression} = any(${sqlBuilder.addParam(property.bid.dates)}::date[])`,
          );
        }
        ...
        if (isoDowValues.length > 0) {
          clauses.push(
            `extract(isodow from ${departingDateExpression}) = any(${sqlBuilder.addParam(isoDowValues)}::int[])`,
          );
        }
        ...
        if (clauses.length === 0) {
          throw new LineholderBidServiceError(
            400,
            `Missing date or day values for ${property.name}.`,
          );
        }
      ...
      if (property.bid.type === "date-range") {
        if (
          ...
          throw new LineholderBidServiceError(
            400,
            `Invalid date range for ${property.name}.`,
          );
        }
    ...
    case 132: {
      if (
        ...
        property.bid.type !== "stepper-range-date"
      ) {
        break;
      }

      const datePlaceholder = sqlBuilder.addParam(property.bid.date);
      const compareBid: Extract<
        PbsPairingBidValue,
        { type: "stepper" | "stepper-range" }
      > =
        property.bid.type === "stepper-date"
          ? {
              type: "stepper",
              value: property.bid.value,
              min: property.bid.min,
              max: property.bid.max,
              operator: property.bid.operator,
            }
          : {
              type: "stepper-range",
    ...
    case 137: {
      ...
      if (pairingType.length === 0) {
        throw new LineholderBidServiceError(
          400,
          `Missing pairing type value for ${property.name}.`,
        );
      }
    ...
    case 163: {
      ...
      if (!context.periodEndDate) {
        throw new LineholderBidServiceError(400, "Roster period range is required for Month-End Carryover.");
      }

      const periodEndPlaceholder = sqlBuilder.addParam(context.periodEndDate);
      const pairingBaseZoneExpression =
        context.pairingBaseZoneExpression ??
      ...
      return buildMonthEndCarryoverClause(
        property.bid,
        carryOutDaysExpression,
        sqlBuilder,
        property.name,
      );
    }
