...
5. **Every topic gets a content regression test** asserting *specific text* (`toContainText('Rule Set')`, `not toContainText('Rule Group')`), never just "renders". Screenshot topics keep the loaded + `naturalWidth ≥ 200px` guard and the **exact image count** (`toHaveCount(n)`).
6. **Title must cover the topic's scope.** If steps outgrow the title, rename or split. Keep `stepCount` honest.
7. **When Help and code disagree and code is wrong, fix code too** and bump `FRONTEND_VERSION`. Don't write Help that contradicts shipped behaviour.
8. **Honour PM answers; rewrite stale sessions wholesale.**

## "NEW" badge for brand-new features (required)

When you add a Help topic for a feature that **just shipped**, flag it so users can spot it: set
`isNew: true` on the topic's `help-data.ts` entry. The left nav (`help-nav.tsx`) renders a small red
**NEW** badge (`data-testid="help-new-badge-<slug>"`) next to any topic whose `isNew` is set — the
in-product equivalent of a "NEW" sticker.

- **Set `isNew: true`** on every topic you add for a brand-new feature (and you *may* set it on an
  existing topic you rewrote because the feature it documents just shipped).
- **Remove it in the next release.** `isNew` is temporary highlighting, not a permanent attribute —
  when the feature is no longer new, delete the `isNew: true` line in the same housekeeping pass you
  bump the version. Don't let stale NEW badges accumulate.
- **Test it:** assert the badge is visible for the new topic and absent on an established one
  (`expect(getByTestId('help-new-badge-<slug>')).toBeVisible()` /
  `expect(getByTestId('help-new-badge-<old-slug>')).toHaveCount(0)`).
...
- [ ] brand-new feature → `isNew: true` on the entry (and a plan to remove it next release)
- [ ] screenshots captured + eyeballed (if any), `HelpScreenshot` count matches the spec
