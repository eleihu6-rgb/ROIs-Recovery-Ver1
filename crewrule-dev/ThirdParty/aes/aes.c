...
//为msg缓冲增加pkcs5Padding
//return len of buf after pkcs5padding, fail for not enough buf return -1
int addPkcs5Padding(unsigned char* buf, int msgLen, int bufsize) {
	int paddingLen = 0;
	int i = 0;
	paddingLen = msgLen % 16 == 0 ? 16 : (16 - msgLen % 16);
	if (bufsize < msgLen + paddingLen) {
		return -1;
	}
	for (i = msgLen; i < msgLen + paddingLen; i++) {
		buf[i] = paddingLen; //pcks5padding, eg:  01 02 03 04 05 06 --> paddingLen=2 --> 01 02 03 04 05 06 02 02
	}
	return msgLen + paddingLen;
}
//remove pkcs5padding from buf
int removePkcs5Padding(unsigned char* buf, int size) {
