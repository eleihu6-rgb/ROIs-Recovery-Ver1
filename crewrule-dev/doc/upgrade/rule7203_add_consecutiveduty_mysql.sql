...
-- mysql
CREATE TABLE `rule_parameter_20260506_1411` LIKE `rule_parameter`;
INSERT INTO `rule_parameter_20260506_1411` SELECT * FROM `rule_parameter`;
...
select rule_id, param_values,
	CONCAT(SUBSTRING_INDEX(param_values, ',', 3),',*,',CONCAT(SUBSTRING_INDEX(SUBSTRING_INDEX(param_values, ',', -7), ',', 4),',*,',SUBSTRING_INDEX(SUBSTRING_INDEX(param_values, ',', -7), ',', -3))) as new_param_values,
	SUBSTRING_INDEX(param_values, ',', 3) as head, 
	SUBSTRING_INDEX(param_values, ',', -7) as tail 
from rule_parameter
...
-- 7203 更新 rule_parameter的table row，在Duty Type后添加*作为Consecutive Duty的默认值
update rule_parameter
