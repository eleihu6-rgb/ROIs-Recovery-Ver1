...
// Test fixture for SIA 5.8 (6) Trans-Atlantic rule tests (Part 2).
class Sia586Part2Test : public ::testing::Test {
protected:
...
TEST_F(Sia586Part2Test, Legal_1LN_in_EUR_if_2LN_in_AME) {
    ...
    // SIN-FRA (2LN rest before next flight)
    ...
    // FRA-JFK
    // Ensure 2 local nights at FRA before departure (22:00-08:00, min 8h; report time 1h).
    ...
    // FRA-SIN (1LN rest in EUR is now allowed)
    time_t fraSinDepUtc = jfkFraArrUtc + 30*3600;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
...
TEST_F(Sia586Part2Test, Illegal_1LN_in_EUR_if_1LN_in_AME) {
    AcopSlipPatternRule rule(nullptr, _input);
    configureRule(rule);
    ...
    // JFK-FRA (1LN rest in AME)
    time_t jfkFraDepUtc = fraJfkArrUtc + 30*3600;
    time_t jfkFraArrUtc = jfkFraDepUtc + 7*3600+45*60;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("JFK", "FRA", jfkFraDepUtc, jfkFraArrUtc, "FLY", true, _ctx));
    ...
    // FRA-SIN (<2LN rest in EUR, which is illegal now)
    time_t fraSinDepUtc = jfkFraArrUtc + 30*3600;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("FRA", "SIN", fraSinDepUtc, fraSinDepUtc + 12*3600, "FLY", true, _ctx));
...
TEST_F(Sia586Part2Test, Legal_Standby_After_1LN_in_AME) {
    AcopSlipPatternRule rule(nullptr, _input);
    configureRule(rule);
    ...
    // Standby after 1LN at JFK
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        time_t sbyStart = fraJfkArrUtc + 30*3600; // after 1LN
        dutySegs.push_back(makeSegment("JFK", "JFK", sbyStart, sbyStart + 6 * 3600, "SBY", false, _ctx));
        duties.push_back(makeDuty(dutySegs, true).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }
    // JFK-FRA (start after 2LN total at JFK)
    time_t jfkFraDepUtc = fraJfkArrUtc + 50*3600;
    time_t jfkFraArrUtc = jfkFraDepUtc + 7*3600+45*60;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
...
TEST_F(Sia586Part2Test, Illegal_Standby_Before_1LN_in_AME) {
    AcopSlipPatternRule rule(nullptr, _input);
    configureRule(rule);
    ...
    // Standby before 1LN at JFK
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        time_t sbyStart = fraJfkArrUtc + 12*3600; // before 1LN
        dutySegs.push_back(makeSegment("JFK", "JFK", sbyStart, sbyStart + 6 * 3600, "SBY", false, _ctx));
        duties.push_back(makeDuty(dutySegs, true).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }
