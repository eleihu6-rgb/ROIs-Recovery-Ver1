...
// Test fixture for SIA 5.8 (4b) Part 3 rule tests.
class Sia584bPart3Test : public ::testing::Test {
protected:
...
// Test Case 1: Illegal, standby on day of arrival
TEST_F(Sia584bPart3Test, Illegal_StandbyOnArrivalDay) {
    ...
    // Day 1: SIN-CDG (flight time ~13.5h)
    time_t sinCdgDepUtc = utcFromString("2025-12-10 00:00:00");
    time_t sinCdgArrUtc = sinCdgDepUtc + 13 * 3600 + 1800;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("SIN", "CDG", sinCdgDepUtc, sinCdgArrUtc, "FLY", true, _ctx));
        duties.push_back(makeDuty(dutySegs).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }

    // Day 1: Standby duties after arrival
    time_t sby1StartUtc = sinCdgArrUtc + 2 * 3600;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("CDG", "CDG", sby1StartUtc, sby1StartUtc + 3 * 3600, "SBY", false, _ctx));
        duties.push_back(makeDuty(dutySegs, true).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }
    time_t sby2StartUtc = sby1StartUtc + 4 * 3600;
     {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("CDG", "CDG", sby2StartUtc, sby2StartUtc + 3 * 3600, "SBY", false, _ctx));
        duties.push_back(makeDuty(dutySegs, true).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }

    // Day 3: CDG-SIN (after 2 local nights)
    time_t cdgSinDepUtc = SIATest::utcFromLocal("2025-12-12 12:00:00", "CDG", _ctx);
    time_t cdgSinArrUtc = cdgSinDepUtc + 13 * 3600;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("CDG", "SIN", cdgSinDepUtc, cdgSinArrUtc, "FLY", true, _ctx));
        duties.push_back(makeDuty(dutySegs).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }
...
// Test Case 2: Illegal, positioning outside of Europe
TEST_F(Sia584bPart3Test, Illegal_PositioningOutsideEurope) {
    AcopSlipPatternRule rule(nullptr, _input);
    configureRule(rule);
    ...
    // Day 1: SIN-FRA
    time_t sinFraDepUtc = utcFromString("2025-12-10 00:00:00");
    time_t sinFraArrUtc = sinFraDepUtc + 13 * 3600;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("SIN", "FRA", sinFraDepUtc, sinFraArrUtc, "FLY", true, _ctx));
        duties.push_back(makeDuty(dutySegs).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }

    // Day 2: [PU] FRA-SIN (positioning outside EUR)
    time_t fraSinDepUtc = SIATest::utcFromLocal("2025-12-11 12:00:00", "FRA", _ctx);
    time_t fraSinArrUtc = fraSinDepUtc + 12 * 3600 + 1800;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("FRA", "SIN", fraSinDepUtc, fraSinArrUtc, "DHD", false, _ctx));
        duties.push_back(makeDuty(dutySegs).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }
...
// Standby can occur on any day within the slip; if standby occurs, the following day may be standby/COP and then requires 1 DO.
// Day 1: SIN-CDG; Day 3: STBY 3h; Day 4: STBY 3h; Day 6: CDG-SIN (Day 5 is day free).
TEST_F(Sia584bPart3Test, Legal_StandbyLaterWithinSlipThenDayOff) {
    AcopSlipPatternRule rule(nullptr, _input);
    configureRule(rule);
    ...
    // Day 3: STBY 3h
    time_t sby1StartUtc = SIATest::utcFromLocal("2025-12-12 10:00:00", "CDG", _ctx);
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("CDG", "CDG", sby1StartUtc, sby1StartUtc + 3 * 3600, "SBY", false, _ctx));
        duties.push_back(makeDuty(dutySegs, true).release());
    ...
    // Day 4: STBY 3h (total standby 6h in 2 periods)
    time_t sby2StartUtc = SIATest::utcFromLocal("2025-12-13 10:00:00", "CDG", _ctx);
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("CDG", "CDG", sby2StartUtc, sby2StartUtc + 3 * 3600, "SBY", false, _ctx));
        duties.push_back(makeDuty(dutySegs, true).release());
    ...
    // Day 6: CDG-SIN (Day 5 is day free)
    time_t cdgSinDepUtc = SIATest::utcFromLocal("2025-12-15 12:00:00", "CDG", _ctx);
    time_t cdgSinArrUtc = cdgSinDepUtc + 13 * 3600;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("CDG", "SIN", cdgSinDepUtc, cdgSinArrUtc, "FLY", true, _ctx));
...
// If standby occurs, the next day may be a COP out of Europe after 06:00 LT without a day free in Europe.
// Day 1: SIN-CDG; Day 3: STBY 2x3h; Day 4: CDG-SIN after 06:00.
TEST_F(Sia584bPart3Test, Legal_StandbyThenDepartOutOfEuropeNextDayAfter0600) {
    AcopSlipPatternRule rule(nullptr, _input);
    configureRule(rule);
    ...
    // Day 3: STBY 2x3h (total 6h, max 2 periods)
    time_t sby1StartUtc = SIATest::utcFromLocal("2025-12-12 10:00:00", "CDG", _ctx);
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("CDG", "CDG", sby1StartUtc, sby1StartUtc + 3 * 3600, "SBY", false, _ctx));
    ...
    // Day 4: CDG-SIN after 06:00 LT, no internal duties on Day 4.
    time_t cdgSinDepUtc = SIATest::utcFromLocal("2025-12-13 10:00:00", "CDG", _ctx);
    time_t cdgSinArrUtc = cdgSinDepUtc + 13 * 3600;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("CDG", "SIN", cdgSinDepUtc, cdgSinArrUtc, "FLY", true, _ctx));
