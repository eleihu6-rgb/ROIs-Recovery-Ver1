...
// SuiteId: 6.11
// Name: CC Layover Rest (NAM Non-ULR Welfare)
// SourceCsvRow: 95
// Status: IMPLEMENTED
// ImplementedCases:
//   - SoftAlertFor1LocalNightRest: Layover of 25h with 1 LN (SEA) triggers a soft violation.
//   - CompliantFor3LocalNightsRest: Layover of 71h with 3 LNs (SEA) is compliant.
// Results:
//   - TODO
...
class SIA_6_11_LayoverRestNamNonUlrTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(SIA_6_11_LayoverRestNamNonUlrTest, SoftAlertFor1LocalNightRest) {
    RuleInput input;
    ...
    input.dbRules.push_back(makeAcopTableBRow(7421001, 1, "NAM_NON_ULR_ANR", "*", "SEA", 2, "*", "*", "*", "*", "*", "25:00", "1", "*", "*", "*", "*", "*", "*", "0", "*"));
    ...
    segStorage.push_back(makeFlight("SIN", "SEA", "2025-12-12 01:00:00", "2025-12-12 15:45:00"));
    auto duty1 = makeDuty({segStorage.back().get()});
    duty1->setActualDropoffMin(_dropoffMin);

    segStorage.push_back(makeFlight("SEA", "SIN", "2025-12-13 19:15:00", "2025-12-14 11:35:00"));
    ...
    if (_violations.size() == 1) {
        ...
        EXPECT_FALSE(std::string(_violations[0]->violation_msg).empty());
    }
}

TEST_F(SIA_6_11_LayoverRestNamNonUlrTest, CompliantFor3LocalNightsRest) {
    RuleInput input;
    // Hard rule: min 25h rest, 1 LN
    input.dbRules.push_back(makeAcopTableBRow(7421001, 1, "NAM_NON_ULR_ANR", "*", "SEA", 2, "*", "*", "*", "*", "*", "25:00", "1", "*", "*", "*", "*", "*", "*", "0", "*"));
    input.dbRules.back().overridebility = "H";
    ...
    segStorage.push_back(makeFlight("SEA", "SIN", "2025-12-15 17:15:00", "2025-12-16 09:35:00"));
    auto duty2 = makeDuty({segStorage.back().get()});
    duty2->setActualPickupMin(_pickupMin);
