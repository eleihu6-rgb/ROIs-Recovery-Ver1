...
// Test fixture for SIA 5.8 (5) Standby rule tests.
class Sia585StandbyTest : public ::testing::Test {
protected:
    ...
    void SetUp() override {
        ...
        // Rule 5.8.(5)
        _input.dbRules.push_back(makeAcopTableBRow(7421001, 1, "5(h)", "SIN-CTYO-LAX-CTYO-SIN", "LAX", 1, "*", "*", "*", "*", "*", "*", "2", "*", "*", "1", "00:00", "2", "6", "*", "*", "*"));
...
TEST_F(Sia585StandbyTest, Legal_Standby_At_LAX) {
    ...
    time_t nrtLaxArrUtc = nrtLaxDepUtc + 10 * 3600; // Arrive LAX 2025-12-03 01:00 PST
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
    ...
    // Standby on day after arrival (Dec 4)
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        time_t sbyStart = SIATest::utcFromLocal("2025-12-04 10:00:00", "LAX", _ctx);
        dutySegs.push_back(makeSegment("LAX", "LAX", sbyStart, sbyStart + 3 * 3600, "SBY", false, _ctx));
        duties.push_back(makeDuty(dutySegs, true).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }
     {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        time_t sbyStart = SIATest::utcFromLocal("2025-12-04 14:00:00", "LAX", _ctx);
        dutySegs.push_back(makeSegment("LAX", "LAX", sbyStart, sbyStart + 3 * 3600, "SBY", false, _ctx));
        duties.push_back(makeDuty(dutySegs, true).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }
...
TEST_F(Sia585StandbyTest, Illegal_Standby_On_Arrival_Day_At_LAX) {
    AcopSlipPatternRule rule(nullptr, _input);
    configureRule(rule);
    ...
    // Standby on day of arrival (Dec 3)
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        time_t sbyStart = SIATest::utcFromLocal("2025-12-03 10:00:00", "LAX", _ctx);
        dutySegs.push_back(makeSegment("LAX", "LAX", sbyStart, sbyStart + 3 * 3600, "SBY", false, _ctx));
        duties.push_back(makeDuty(dutySegs, true).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }
...
TEST_F(Sia585StandbyTest, Legal_1LN_At_TYO2_After_LAX_With_Standby) {
    AcopSlipPatternRule rule(nullptr, _input);
    configureRule(rule);
    ...
    // Standby at LAX
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        time_t sbyStart = SIATest::utcFromLocal("2025-12-04 10:00:00", "LAX", _ctx);
        dutySegs.push_back(makeSegment("LAX", "LAX", sbyStart, sbyStart + 6 * 3600, "SBY", false, _ctx));
        duties.push_back(makeDuty(dutySegs, true).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }
    ...
    // NRT-SIN with legal 1LN rest
    time_t nrtSinDepUtc = laxNrtArrUtc + 30 * 3600;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("NRT", "SIN", nrtSinDepUtc, nrtSinDepUtc + 7 * 3600, "FLY", true, _ctx));
        duties.push_back(makeDuty(dutySegs).release());
...
TEST_F(Sia585StandbyTest, Illegal_LessThan1LN_At_TYO2_After_LAX_With_Standby) {
    AcopSlipPatternRule rule(nullptr, _input);
    configureRule(rule);
    ...
    // NRT-SIN with illegal <1LN rest
    time_t nrtSinDepUtc = laxNrtArrUtc + 15 * 3600;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("NRT", "SIN", nrtSinDepUtc, nrtSinDepUtc + 7 * 3600, "FLY", true, _ctx));
        duties.push_back(makeDuty(dutySegs).release());
