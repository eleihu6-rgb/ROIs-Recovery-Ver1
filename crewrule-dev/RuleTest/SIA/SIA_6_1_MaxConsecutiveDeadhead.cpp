...
// SuiteId: 6.1
// Name: Maximum number of consecutive deadhead sectors
// SourceCsvRow: 6.1.,Maximum number of consecutive deadhead sectors
// Status: IMPLEMENTED
// ImplementedCases:
//   - Case #1: With max consecutive deadhead value = 2, a duty containing 3 consecutive DHD legs is illegal.
//   - Case #2: With max consecutive deadhead value = 3, the same duty is legal.
// Results:
//   - pass 2 out of 2 (skipped 0, disabled 0).
//   - Notes: last run 2025-12-04T21:51:21Z
// RemainingWork:
//   - Confirm that consecutive DHD counting matches SIA expectations when long transits or complex patterns are present.
...
// SIA 6.1 – Maximum number of consecutive deadhead sectors
// Best-fit rule: rule2004 (field maxNrConsecutiveDhd).
...
class SIA_6_1_MaxConsecutiveDeadheadTest : public ::testing::Test {
protected:
    SIA_6_1_MaxConsecutiveDeadheadTest()
    ...
    std::unique_ptr<Duty> buildDutyWithThreeConsecutiveDeadheads() {
        _segStore.clear();
...
// Case #1 (6.1): max consecutive DHD = 2, actual consecutive DHD legs = 3 -> illegal.
TEST_F(SIA_6_1_MaxConsecutiveDeadheadTest, ThreeConsecutiveDeadheadsExceedLimitOfTwo) {
    Rule2004Config cfg;
    cfg.maxConsecutiveDhd = "2";
    auto rule = makeRule2004(cfg);

    auto duty = buildDutyWithThreeConsecutiveDeadheads();
    std::vector<Duty*> duties{duty.get()};

    EXPECT_FALSE(_checker.checkDutyLimitation(duties, rule.get(), "SIN"));
}

// Case #2 (6.1): max consecutive DHD = 3, actual consecutive DHD legs = 3 -> legal.
TEST_F(SIA_6_1_MaxConsecutiveDeadheadTest, ThreeConsecutiveDeadheadsWithinLimitOfThree) {
    Rule2004Config cfg;
    cfg.maxConsecutiveDhd = "3";
    auto rule = makeRule2004(cfg);
