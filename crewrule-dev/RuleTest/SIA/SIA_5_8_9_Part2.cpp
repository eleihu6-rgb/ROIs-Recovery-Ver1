...
// Test fixture for SIA 5.8 (9) Africa (Part 2) rule tests.
class Sia589AfricaPart2Test : public ::testing::Test {
protected:
    ...
    void SetUp() override {
        ...
        // Rule text: (For scheduled departures from SIN between midnight and 0600 hours)
        const std::string REPORT_TIME_WINDOW = "00:00-06:00";

        // Rule 9(a)(i).1: operate JNB-CPT-JNB or JNB-DUR-JNB after 1LN
        _input.dbRules.push_back(makeAcopTableBRow(7421001, 1, "9(a)(i).1", "SIN-JNB-SIN", "JNB", 1, "*", "*", REPORT_TIME_WINDOW, "*", "*", "*", "1", "*", "*", "*", "*", "*", "*", "Duty IN (JNB-CPT-JNB, JNB-DUR-JNB)", "*", "*"));
        // Rule 9(a)(i).2: standby
        _input.dbRules.push_back(makeAcopTableBRow(7421002, 2, "9(a)(i).2", "SIN-JNB-SIN", "JNB", 1, "*", "*", REPORT_TIME_WINDOW, "*", "*", "*", "1", "*", "*", "*", "*", "2", "6", "*", "*", "*"));
        // Rule 9(a)(i).3: position back to Singapore after 24 hours, which includes one local night.
        _input.dbRules.push_back(makeAcopTableBRow(7421003, 3, "9(a)(i).3", "SIN-JNB-SIN", "JNB", 1, "MVO|FLY", "MVP", REPORT_TIME_WINDOW, "*", "*", "24", "1", "*", "*", "*", "*", "*", "*", "*", "*", "*"));
        // Rule 9(a)(ii): operate JNB-SIN after 24 hours, which includes one local night after positioning to JNB
        _input.dbRules.push_back(makeAcopTableBRow(7421004, 4, "9(a)(ii)", "SIN-JNB-SIN", "JNB", 1, "MVP", "MVO|FLY", REPORT_TIME_WINDOW, "*", "*", "24", "1", "*", "*", "*", "*", "*", "*", "*", "*", "*"));
...
// Test Case 1: Legal, 1LN after positioning to JNB
// Rule: 9(a)(ii)
TEST_F(Sia589AfricaPart2Test, Legal_Position_To_JNB_Then_Operate) {
    ...
    // Day 1: [PU] SQ478 SIN-JNB 0210-0650
    ...
    time_t sinJnbArrUtc = sinJnbDepUtc + 10*3600 + 40*60;
    {
        ...
        dutySegs.push_back(makeSegment("SIN", "JNB", sinJnbDepUtc, sinJnbArrUtc, "MVP", false, _ctx));
        duties.push_back(makeDuty(dutySegs).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }
    // Day 3: SQ479 JNB-SIN 1345-0610 (rest is > 24h and includes 1 LN)
...
// Test Case 2: Legal, position to SG after 24h incl 1LN
// Rule: 9(a)(i).3
TEST_F(Sia589AfricaPart2Test, Legal_Operate_To_JNB_Then_Position) {
    AcopSlipPatternRule rule(nullptr, _input);
    configureRule(rule);
    ...
    // Day 3: [PU] SQ479 JNB-SIN 1345-0610 (rest is > 24h and includes 1 LN)
    time_t jnbSinDepUtc = SIATest::utcFromLocal("2025-12-03 13:45:00", "JNB", _ctx);
    time_t jnbSinArrUtc = jnbSinDepUtc + 10*3600 + 25*60;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("JNB", "SIN", jnbSinDepUtc, jnbSinArrUtc, "MVP", false, _ctx));
        duties.push_back(makeDuty(dutySegs).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }
