...
DBRule makeAcopTableBRow(long long ruleId,
                         ...
                         const std::string& dayOffAfterDuty,
    ...
    rule.params["DO after duty"] = dayOffAfterDuty;
...
// Test fixture for SIA 5.8 (4di) SIN-EUR Positioning rule tests.
class Sia584diSineurPositioningTest : public ::testing::Test {
protected:
    ...
    void SetUp() override {
        _ctx = SIATest::buildCrewDataContext({
            ...
            {"MAN", 0, "Europe/London"}, // UTC+0, no DST for simplicity
        ...
        // LocationMatchUtils region token "REUR" matches DBAirport.category == "EUR".
        addAirport("MAN", "GB", "EUR");
        addAirport("AMS", "NL", "EUR");
        
        // Rule (4)(d)(i)
        _input.dbRules.push_back(makeAcopTableBRow(7421001, 1, "(4)(d)(i)", "SIN-REUR-SIN", "REUR", 1, "MVP", "*", "*", "24", "1", "*", "*"));
        
        // Rule (4)(d)(ii)
        _input.dbRules.push_back(makeAcopTableBRow(7421001, 2, "(4)(d)(ii)", "SIN-REUR-SIN", "REUR", 1, "MVP", "*", "*", "*", "1", "POS|FDP<=6", "1"));
    }
...
TEST_F(Sia584diSineurPositioningTest, Legal_SufficientRest24h1LN) {
    ...
    // Day 1: [PU] SIN-MAN. Flight duration ~14h.
    time_t posDepUtc = utcFromString("2025-12-01 10:00:00");
    time_t posArrUtc = posDepUtc + 14 * 3600;
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("SIN", "MAN", posDepUtc, posArrUtc, "DHD", false, _ctx));
        duties.push_back(makeDuty(dutySegs).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }

    // Day 3: MAN-SIN. This provides more than 24h rest and includes a local night.
    // Rest starts after Duty 1 release (arrival + 30m debrief + 60m transport)
    // Release time = posArrUtc + 90 * 60 = 01-DEC 10:00 + 14h + 1.5h = 02-DEC 01:30 UTC
    // Next report time on Day 3, e.g., 02-DEC 10:00 UTC, gives rest > 24h.
    time_t fltDepUtc = utcFromString("2025-12-03 10:00:00");
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("MAN", "SIN", fltDepUtc, fltDepUtc + 14 * 3600, "FLY", true, _ctx));
        duties.push_back(makeDuty(dutySegs).release());
        for (auto& s : dutySegs) segStorage.push_back(std::move(s));
    }
    ...
    // This should match rule (4)(d)(i) and be legal.
    EXPECT_TRUE(rule.CheckRule(&pairing));
}

TEST_F(Sia584diSineurPositioningTest, Illegal_LessThan24hRestAndLongFDP) {
    ...
    // Day 1: [PU] SIN-MAN.
    time_t posDepUtc = utcFromString("2025-12-01 18:00:00");
    time_t posArrUtc = posDepUtc + 14 * 3600; // Arrives 08:00 UTC on Dec 2nd
    {
        std::vector<std::unique_ptr<Segment>> dutySegs;
        dutySegs.push_back(makeSegment("SIN", "MAN", posDepUtc, posArrUtc, "DHD", false, _ctx));
        duties.push_back(makeDuty(dutySegs).release());
    ...
    // Day 2: MAN-SIN. Rest is < 24h and has 0 local nights.
    // Duty 1 release: 02-DEC 08:00 + 1.5h = 09:30 UTC.
    // Next report time 21:00 UTC on Dec 2nd gives rest = 11:30 (no local night).
    ...
    // This should trigger rule (4)(d)(i) and fail because slip < 24h and has 0 local nights.
