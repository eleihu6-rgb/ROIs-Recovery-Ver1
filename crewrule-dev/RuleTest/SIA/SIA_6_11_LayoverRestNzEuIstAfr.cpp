...
// SuiteId: 6.11
// Name: CC Layover Rest (NZ, EU, IST & AFR Welfare)
// SourceCsvRow: 94
// Status: IMPLEMENTED
// ImplementedCases:
//   - SoftAlertFor1LocalNightRest: Checks that a layover of ~23h with 1 LN triggers a soft violation.
//   - CompliantFor2LocalNightsRest: Checks that a layover of ~47h with 2 LNs is compliant.
// Results:
//   - TODO
...
class SIA_6_11_LayoverRestNzEuIstAfrTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(SIA_6_11_LayoverRestNzEuIstAfrTest, SoftAlertFor1LocalNightRest) {
    RuleInput input;
    // Soft rule: min 2 LN
    input.dbRules.push_back(makeAcopTableBRow(7421004, 4, "NZ_Welfare", "*", "CHC", 1, "*", "*", "*", "*", "*", "46:00", "2", "*", "*", "*", "*", "*", "*", "0", "*"));
    input.dbRules.back().overridebility = "S";
    // Hard rule: min 1 LN
    input.dbRules.push_back(makeAcopTableBRow(7421003, 3, "NZ_ANR", "*", "CHC", 2, "*", "*", "*", "*", "*", "22:00", "1", "*", "*", "*", "*", "*", "*", "0", "*"));
    input.dbRules.back().overridebility = "H";
    DBRule controlRow{};
    controlRow.idRule = 7421003;
    ...
    segStorage.push_back(makeFlight("SIN", "CHC", "2025-12-12 11:50:00", "2025-12-12 21:40:00"));
    auto duty1 = makeDuty({segStorage.back().get()});
    duty1->setActualDropoffMin(_dropoffMin);

    segStorage.push_back(makeFlight("CHC", "SIN", "2025-12-13 23:00:00", "2025-12-14 09:40:00"));
    ...
    if (_violations.size() == 1) {
        EXPECT_EQ(_violations[0]->ruleParamId, 742100000 + 4); // Soft violation
    }
}

TEST_F(SIA_6_11_LayoverRestNzEuIstAfrTest, CompliantFor2LocalNightsRest) {
    RuleInput input;
    // Soft rule: min 2 LN
    input.dbRules.push_back(makeAcopTableBRow(7421004, 4, "NZ_Welfare", "*", "CHC", 1, "*", "*", "*", "*", "*", "46:00", "2", "*", "*", "*", "*", "*", "*", "0", "*"));
    input.dbRules.back().overridebility = "S";
    ...
    segStorage.push_back(makeFlight("CHC", "SIN", "2025-12-14 23:00:00", "2025-12-15 09:40:00"));
    auto duty2 = makeDuty({segStorage.back().get()});
    duty2->setActualPickupMin(_pickupMin);
