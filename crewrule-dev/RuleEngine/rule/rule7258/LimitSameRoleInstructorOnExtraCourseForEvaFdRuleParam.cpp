...
void LimitSameRoleInstructorOnExtraCourseForEvaFdRuleParam::ParseParam(const std::string &paramString) {
...
void LimitSameRoleInstructorOnExtraCourseForEvaFdRuleParam::ParseParam(const DBRule& dbRule) {
	...
	for (map<string, string>::iterator iter = parameter.begin(); iter != parameter.end(); iter++)
	{
		...
		if (header == "BASES") {
			...
			if (headeValue != RuleParamConstant::ALL) {
				split(headeValue, '|', _trainingRoles);
			}
		}
...
bool LimitSameRoleInstructorOnExtraCourseForEvaFdRuleParam::MatchCrewQualification(std::shared_ptr<CREW> crew, const time_t& checkedStartTime, const time_t& checkedEndTime) const {
...
bool LimitSameRoleInstructorOnExtraCourseForEvaFdRuleParam::MatchCourse(const std::shared_ptr<TmCourse>& tmCourse) const {
	if (this->_courseCodes.empty()) {
		return true;
	}
	string& courseCode = tmCourse->courseCode;
	return std::find(this->_courseCodes.begin(), this->_courseCodes.end(), courseCode) != this->_courseCodes.end();
}