...
#ifndef _TASKONLYBEOPERATEDBYFILTEREDCREWFORPRRULEPARAM_H_
#define _TASKONLYBEOPERATEDBYFILTEREDCREWFORPRRULEPARAM_H_
...
class TaskOnlyBeOperatedByFilteredCrewForPRRule;

class TaskOnlyBeOperatedByFilteredCrewForPRRuleParam : public RuleParam {
	friend class TaskOnlyBeOperatedByFilteredCrewForPRRule;
private:
    explicit TaskOnlyBeOperatedByFilteredCrewForPRRuleParam(const Rule* rule) :RuleParam(rule) {};
	...
	//航班所在RosterGround或Pairing的attribute列表。多个值使用“|”分隔并支持*通配
...
#endif //_TASKONLYBEOPERATEDBYFILTEREDCREWFORPRRULEPARAM_H_
