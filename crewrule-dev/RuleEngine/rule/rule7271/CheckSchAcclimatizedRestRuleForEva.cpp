/**
 * @file CheckSchAcclimatizedRestRuleForEva.cpp
 * @brief
 * @author jiaxin.jin
...
#include "../RuleSytem.h"
#include "CheckSchAcclimatizedRestRuleForEva.h"
#include "UtilFunc.h"
#include "Utility.h"
...
bool CheckSchAcclimatizedRestRuleForEva::CheckRule(const std::vector<const ROSTER*>& rosters) const {
			...
			if (rosters[i]->pairing) {
				...
				if (size > 1) {
					...
					for (size_t z = 0; z < size - 1; z++) {
						...
						if (!SchDutyUtils::existFleetsInDuty(duty) && !SchDutyUtils::existFleetsInDuty(nextDuty)) {
							//当前Duty和下一个Duty没有货机机型，则不用按计划时间进行检查
							continue;
						}

						int scheduleRest = SchDutyUtils::GetScheduleRestMinutesIncludePickupAndDropoff(duty, nextDuty, this->GetDataContext());
						...
						{
							continuousLayoverMinutes += scheduleRest;
							lastTimezoneGapEndUtcAtBase = SchDutyUtils::GetDutySchEndTimeUtc(duties[z + 1]) + duties[z + 1]->getMinDropoff() * 60;

							const auto& lastSeg = duties[z + 1]->getLastSegment();
							if (lastSeg)
								lastTimezoneGapEndUtcAtBase = lastSeg->getEndTimeUtcSch() + 30 * 60;
							...
							if (isOverlap)
							{
								...
								if (!rosters[j]->pairing)  //GROUND ROSTER
								{
									...
									if (find_it == assignmentException.end())
									{
										...
										_ruleViolation.SetParam("schRest", Utility::GetInstancePtr()->getClock_format(layover / 60, layover % 60));
								...
								else //PAIRING
								{
									...
									{
										time_t minRestEndAtBast = endUtcAtBase + ruleParam._minRestAtHomeBase * 60;
										bool isSegOverlap = Utility::GetInstancePtr()->isTimeOverlap(endUtcAtBase, minRestEndAtBast, duty->getStartTimeUtcSch(), duty->getEndTimeUtcSch());
										if (minRestEndAtBast == duty->getStartTimeUtcSch())
											isSegOverlap = false;
										if (isSegOverlap)
										{
											...
											if (assignment.size() == 0)
												assignment = "FLY";
											string arr = duty->getArrStation();
											vector<string>::iterator find_it = std::find(assignmentException.begin(), assignmentException.end(), assignment);
											//if ((find_it == vDutyException.end() && base == rosters[i]->getBase())
											//|| (base != rosters[i]->getBase() && std::find(vDutyException.begin(), vDutyException.end(), duty_code) == vDutyException.end()))
											if ((base == arr && find_it == assignmentException.end())
												|| (base != arr && std::find(assignmentException.begin(), assignmentException.end(), duty_code) == assignmentException.end()))
											{
												if (this->_application == ROSTER_OPTIMIZER)
												{
													/*if (!hasOptimizedNewRosterInRange(rosters, endUtcAtBase, endUtcAtBase + range * 60))
...
void CheckSchAcclimatizedRestRuleForEva::ThrowRuleViolation(const ROSTER* roster) const {
	...
	const auto& schRest = _ruleViolation.GetParam("schRest");
	...
	string msgViolation = "Since the crew member's next assigned duty after returning to base is not of the " + assignments + " duty type, the current rest period(" + schRest + ") at home base must be at least " + minrest + " hours.";
	...
	rv->operation_result.insert(pair<string, string>("scheduleRest", schRest));
...
void CheckSchAcclimatizedRestRuleForEva::ParseParam(const InputType& input) {
	//add by hexd 添加DBRule支持
	for (const auto& dbRule : input.dbRules) {
		_ruleParams.emplace_back(CheckSchAcclimatizedRestRuleParamForEva(this));
