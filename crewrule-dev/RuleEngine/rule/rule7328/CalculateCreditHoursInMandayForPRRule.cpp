#include "CalculateCreditHoursInMandayForPRRule.h"
#include <cfloat>
#include <algorithm>
#include "../RuleSytem.h"
#include "UtilFunc.h"
...
std::tuple<map<time_t, MandayCreditHour>, map<long long, CreditHour>, map<long long, map<long long, CreditHour>>> CalculateCreditHoursInMandayForPRRule::Calculate(std::vector<const ROSTER*>& rosters) {
		...
		if (roster->pairing == nullptr) {
			...
			for (auto& duty : roster->pairing->getDutyVec()) {
				for (size_t i = 0; i < duty->getNumSegments(); i++) {
					auto segment = duty->getSegment(i);
					segmentIdMap.insert(std::make_pair(segment->getSegmentId(), segment));
					for (const auto& ruleParam : _ruleParams) {
						if (ruleParam.MatchParam(roster, duty, segment, i)) {
							auto iter = ruleActivityMap.find(&ruleParam);
							if (iter == ruleActivityMap.end()) {
	...
	for (auto& ruleActivity : ruleActivityMap) {
		auto tmpMapCrewMandayDay = CalculateCreditDay(allSegmentMandayActivities, allGroundRosterMandayActivities, allAssignmentMapCrewMandayDay, ruleActivity.second, offsetTZMinutes, crew, *(CreditHoursDefinitionForPRRuleParam*)(ruleActivity.first));
...
//通过飞时计算Credit
vector<std::shared_ptr<MandayActivity>> CalculateCreditHoursInMandayForPRRule::GetCreditActivityByBLH(const vector<const Activity*>& activities, const int offsetTZMinutes, const CreditHoursDefinitionForPRRuleParam& ruleParam) {
	...
	for (auto& activity : activities) {
		...
		mandayActivity->setPercent(ruleParam._creditPercentage);
...
map<time_t, std::tuple<double, double>> CalculateCreditHoursInMandayForPRRule::CalculateCreditDay(vector<std::shared_ptr<MandayActivity>>& allSegmentMandayActivities, vector<std::shared_ptr<MandayActivity>>& allGroundRosterMandayActivities, map<string, map<time_t, std::tuple<double, double>>>& allAssignmentMapCrewMandayDay,
...
void CalculateCreditHoursInMandayForPRRule::CalculateCreditDayByExpression(map<time_t, double>& mapCrewMandayDay, const std::shared_ptr<CREW>& crew, const CreditHoursDefinitionForPRRuleParam& ruleParam) const {
	for (auto& pair : mapCrewMandayDay) {
		pair.second = pair.second / 60.0; //单位：分钟
	}
}

void CalculateCreditHoursInMandayForPRRule::Merge(map<string, map<time_t, std::tuple<double, double>>>& allAssignmentMapCrewMandayDay, const string& assignment, const map<time_t, std::tuple<double, double>>& mapAssignmentMandayDay) const {
...
map<string, vector<std::shared_ptr<MandayActivity>>> CalculateCreditHoursInMandayForPRRule::GetMandayActivitiesByAssignment(const vector<std::shared_ptr<MandayActivity>>& mandayActivities) const {
...
string CalculateCreditHoursInMandayForPRRule::GetTimeType(const Activity* activity, const int offsetTZMinutes, const CreditHoursDefinitionForPRRuleParam& ruleParam) const {
	return string("ACT");
}

void CalculateCreditHoursInMandayForPRRule::ParseParam(const InputType& input) {
	//add by hexd 添加DBRule支持
	for (const auto& dbRule : input.dbRules) {
		_ruleParams.emplace_back(CreditHoursDefinitionForPRRuleParam(this));
