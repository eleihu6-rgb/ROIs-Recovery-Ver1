...
void OffDutyPeriodsForCumulativeIn7DaysRuleParam::ParseParam(const std::string &paramString) {
    ...
    for (int i = 0; i < totalNumParam; ++i) {
        ...
        if (!substr.empty()) {
			...
			case enum_to_underlying(ParamLocation::MIN_LOCAL_NIGHT):
				_minLocalNight = atoi(substr.c_str());
				break;
...
void OffDutyPeriodsForCumulativeIn7DaysRuleParam::ParseParam(const DBRule& dbRule) {
	...
	for (map<string, string>::iterator iter = parameter.begin(); iter != parameter.end(); iter++)
	{
		...
		if (header == "BASES") {
			...
			_minLocalNight = atoi(headeValue.c_str());
		}
...
bool OffDutyPeriodsForCumulativeIn7DaysRuleParam::MatchCrewQualification(std::shared_ptr<CREW> crew, const time_t& checkedStartTime, const time_t& checkedEndTime) const {
...
bool OffDutyPeriodsForCumulativeIn7DaysRuleParam::MatchAssignmentGroups(const string& assignment, const std::shared_ptr<CrewDataContext>& dbData) const {
	for (auto assignmentGroup : _assignmentGroups) {
