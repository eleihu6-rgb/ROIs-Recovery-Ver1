...
void LimitConsecutiveWoclForCARSRuleParam::ParseParam(const std::string&) {}

void LimitConsecutiveWoclForCARSRuleParam::ParseParam(const DBRule& dbRule) {
	...
	for (map<string, string>::iterator iter = parameter.begin(); iter != parameter.end(); iter++) {
		...
		else if (header == "MAX CONSECUTIVE WOCLS")
			_maxConsecutiveWoclNumber = atoi(headeValue.c_str());
	}
}

bool LimitConsecutiveWoclForCARSRuleParam::MatchCrewQualification(std::shared_ptr<CREW> crew, const time_t& checkedStartTime, const time_t& checkedEndTime) const {
