...
void SchMinWOCLAtLayoverStationForEvaFdRuleParam::ParseParam(const std::string &paramString) {
...
void SchMinWOCLAtLayoverStationForEvaFdRuleParam::ParseParam(const DBRule& dbRule) {
...
bool SchMinWOCLAtLayoverStationForEvaFdRuleParam::ignoreMinRestAtLayover() const {
	return _strMinRestAtLayover.empty() || _strMinRestAtLayover == RuleParamConstant::ALL;
}

//是否忽略"Layover最小WOCL次数"参数，返回true-忽略，false-未忽略
bool SchMinWOCLAtLayoverStationForEvaFdRuleParam::ignoreMinWOCLAtLayover() const {
	return _strMinSleepCycles.empty() || _strMinSleepCycles == RuleParamConstant::ALL;
}

bool SchMinWOCLAtLayoverStationForEvaFdRuleParam::MatchCrewQualification(std::shared_ptr<CREW> crew, const time_t& checkedStartTime, const time_t& checkedEndTime) const {
...
bool SchMinWOCLAtLayoverStationForEvaFdRuleParam::MatchNextDutyComposition(const Duty* nextDuty) const {
...
bool SchMinWOCLAtLayoverStationForEvaFdRuleParam::MatchSegmentAssignments(const Segment& segment) const {
...
bool SchMinWOCLAtLayoverStationForEvaFdRuleParam::MatchMaxSegmentFlightTime(const Duty* currDuty) const {
	...
	for (auto& segment : currDuty->getSegments()) {
		if (MatchSegmentAssignments(*segment)) {
			int blh = SegmentUtils::GetSchBlkMinutes(*segment);
...
//判断是否在基地
bool SchMinWOCLAtLayoverStationForEvaFdRuleParam::MatchPairingEndAirports(const Pairing& pairing) const {
...
bool SchMinWOCLAtLayoverStationForEvaFdRuleParam::CheckMinRestAtLayover(const Duty* currDuty, const Duty* nextDuty) const {
	if (nextDuty == nullptr) {
		return true;
	}
	...
	if (scheduleRest >= _minRestAtLayoverMinutes) {
		valid = true;
	}
	return valid;
}

bool SchMinWOCLAtLayoverStationForEvaFdRuleParam::CheckMinWOCLAtLayover(const Duty* currDuty, const Duty* nextDuty) const {
	if (nextDuty == nullptr) {
		return true;
	}
	...
	time_t startTime = currDuty->getEndTimeUtcSch() + currDuty->getMinDropoff() * 60 + offsetTZMinutes * 60;
	time_t endTime = nextDuty->getStartTimeUtcSch() - nextDuty->getMinPickup() * 60 + offsetTZMinutes * 60;
...
bool SchMinWOCLAtLayoverStationForEvaFdRuleParam::MatchParam(const Duty* currDuty, const Duty* nextDuty, const Pairing* pairing) const {
...
//检查是否满足参数
int SchMinWOCLAtLayoverStationForEvaFdRuleParam::CheckParam(const Duty* currDuty, const Duty* nextDuty) const {
	if (nextDuty == nullptr) {
