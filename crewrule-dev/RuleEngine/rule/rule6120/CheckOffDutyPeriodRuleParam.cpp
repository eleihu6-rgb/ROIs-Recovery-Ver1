/**
 * @file CheckOffDutyPeriodRuleParam.h
 * @brief
 * @author xuedong.he
...
void CheckOffDutyPeriodRuleParam::ParseParam(const std::string &paramString) {
    ...
    for (int i = 0; i < totalNumParam; ++i) {
        ...
        if (!substr.empty()) {
			...
			case enum_to_underlying(ParamLocation::DISPLACEMENT_ADJUSTMENT_REFERENCE_TIME):
				...
				break;
			case enum_to_underlying(ParamLocation::IGNORE_MRT_OVERIDE):
				_ignoreMRTOverride = (substr.empty() || substr == RuleParamConstant::ALL) ? nullptr : std::make_shared<bool>(substr == RuleParamConstant::YES);
				break;
...
void CheckOffDutyPeriodRuleParam::ParseParam(const DBRule& dbRule) {
	...
	for (map<string, string>::iterator iter = parameter.begin(); iter != parameter.end(); iter++)
	{
		...
		//Bases, Ranks, Fleets, Duty Assignments, Duty TZ Diff, Sector BLH Ranges, Encroach Time, Is Home Base, Acclimatized State, Benchmark, Period Thredhold, Rest Time, FDP Exceeded Hours, FDP Times, Displacement Adjustment Reference Time, Ignore MRT Override(Y/N)
		...
		else if (header == "IGNORE MRT OVERRIDE(Y/N)") {
			_ignoreMRTOverride = (headeValue.empty() || headeValue == RuleParamConstant::ALL) ? nullptr : std::make_shared<bool>(headeValue == RuleParamConstant::YES);
		}
...
bool CheckOffDutyPeriodRuleParam::MatchCrewQualification(std::shared_ptr<CREW> crew, const time_t& checkedStartTime, const time_t& checkedEndTime) const {
...
bool CheckOffDutyPeriodRuleParam::MatchDutyAssignments(const Duty& duty) const {
...
bool CheckOffDutyPeriodRuleParam::MatchDutyTzDiff(const Duty& duty) const {
...
//判断Sector飞时
bool CheckOffDutyPeriodRuleParam::MatchSectorBlhRanges(const Duty& duty) const {
...
bool CheckOffDutyPeriodRuleParam::MatchEncroachTimeRanges(const Segment* segment) const {
...
//判断是否在基地
bool CheckOffDutyPeriodRuleParam::MatchHomeBase(const Duty& duty, const std::string& base, const SharedPtr<CrewDataContext>& dbData) const {
...
//判断是否在适应期
bool CheckOffDutyPeriodRuleParam::MatchAcclimatizedState(const Duty* currDuty) const {
	if (currDuty == nullptr) {
...
//判断是否匹配位移方向
bool CheckOffDutyPeriodRuleParam::MatchDirection(const string& direction) const {
	return (_direction == RuleParamConstant::ALL) || (direction == _direction);
}

//判断是否匹配FDP阈值
bool CheckOffDutyPeriodRuleParam::MatchFDPThredhold(const Duty& duty, const int dutyFdpMinutes) const {
...
//bool CheckOffDutyPeriodRuleParam::MatchParam(const std::string& composition, const long &reportTime, bool isAugment) const {
//
...
bool CheckOffDutyPeriodRuleParam::CheckRule(const Duty* currDuty, const Duty* nextDuty, const int minRest) const {
	if (nextDuty == nullptr) {
		return true;
	}
...
bool CheckOffDutyPeriodRuleParam::CheckRule(const Duty* currDuty, const WorkPeriod* nextWorkPeriod, const int minRest) const {
	...
	if ((_ignoreMRTOverride == nullptr || !*_ignoreMRTOverride) && nextWorkPeriod != nullptr && AssignmentUtils::IsAssignmentMrtOveride(currDuty->getAssignment(), nextWorkPeriod->GetRoster()->qualifier, this->GetRule()->GetDataContext())) {
		return true;
	}
