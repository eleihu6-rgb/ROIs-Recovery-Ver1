...
#ifndef _CHECKOFFDUTYPERIODRULEPARAM_H_
#define _CHECKOFFDUTYPERIODRULEPARAM_H_
...
class CheckOffDutyPeriodRule;

class CheckOffDutyPeriodRuleParam : public RuleParam {
friend class CheckOffDutyPeriodRule;
private:
    explicit CheckOffDutyPeriodRuleParam(const Rule* rule) :RuleParam(rule) {};
	...
	//是否忽略MRT Override，取值：Y/N。Y: 忽略MRT Override配置(即配置MRT Override也会告警)。N: 不忽略MRT Override配置(默认值）
	shared_ptr<bool> _ignoreMRTOverride{ nullptr };
...
#endif //_CHECKOFFDUTYPERIODRULEPARAM_H_
