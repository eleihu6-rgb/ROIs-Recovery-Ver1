...
#ifndef _CALCULATEFDPEXTENSIONFORCCFORHXRULEPARAM_H_
#define _CALCULATEFDPEXTENSIONFORCCFORHXRULEPARAM_H_
...
class CalculateFdpExtensionForCcForHXRule;

class CalculateFdpExtensionForCcForHXRuleParam : public RuleParam {
	friend class CalculateFdpExtensionForCcForHXRule;
private:
	explicit CalculateFdpExtensionForCcForHXRuleParam(const Rule* rule) :RuleParam(rule) {};
	...
	int _preRestDurationMinutesLower{0};
	int _preRestDurationMinutesUpper{0};
	...
	//降落时，最小同时在线的机组成员数。格式：整数，支持*通配表示忽略
	std::string _minConcurrentCrewAtLandingStr{};
	int _minConcurrentCrewAtLanding{};
	...
	//Extended To FDP。格式：HH:mm,支持*通配表示忽略
	std::string _extendedToFdp{};
	int _extendedToFdpMinutes{};
...
private:
	...
	int maxLandingCrewRestTime(int totalFlightTime, int crewNum, int restFacility) const;

	int CalculateOptimizedRestInFlightByLandingCrew(vector<int> maxWorkingTimesInFlight, int flightTime, int crewNum, int restFacility) const;

	int GetCrewNumber(const Segment* segment, const Duty* duty) const;

	int GetFacilityNumber(const Segment* segment) const;

	int GetMinRestInFlightByLandingCrew(vector<int> restsTimeInFlight) const;

	int GetMaxFdp(const Duty* duty, int maxFdp) const;;
};

#endif //_CALCULATEFDPEXTENSIONFORCCFORHXRULEPARAM_H_
