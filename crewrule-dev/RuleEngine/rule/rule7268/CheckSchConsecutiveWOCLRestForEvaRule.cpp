#include "CheckSchConsecutiveWOCLRestForEvaRule.h"
...
bool CheckSchConsecutiveWOCLRestForEvaRule::CheckRule(const std::vector<const ROSTER*>& rosters) const {
	...
	auto tmpWorkPeriods = WorkPeriod::GetWorkFDPPeriodsBySch(rosters, this->_dbData);
...
bool CheckSchConsecutiveWOCLRestForEvaRule::CheckRest(const std::shared_ptr<WorkPeriod>& beforeDuty, const std::shared_ptr<WorkPeriod>& afterDuty, const int minRest, const CheckSchConsecutiveWOCLRestForEvaRuleParam& ruleParam) const {
	bool isValid = true;
	time_t next_start = afterDuty->GetStartTimeUTC();	
	time_t before_end = beforeDuty->GetEndTimeUTC();

	bool next_existFleet = false;
	bool before_existFleet = false;
	...
	if (beforeDuty->GetWorkType() == FltDuty && beforeDuty->GetRoster()) {
		for (const auto& duty : beforeDuty->GetRoster()->pairing->getDutyVec()) {
			if (to_string(duty->getDutyId()) == beforeDuty->GetKey()) {
				//获得当前Duty是否存在货机机型（EVAj计划时间)
				before_existFleet = SchDutyUtils::existFleetsInDuty(duty);
			}
		}
	}
	...
	if (afterDuty->GetWorkType() == FltDuty && afterDuty->GetRoster()) {
		for (const auto& duty : afterDuty->GetRoster()->pairing->getDutyVec()) {
			if (to_string(duty->getDutyId()) == afterDuty->GetKey()) {
				//获得下一个Duty是否存在货机机型（EVAj计划时间)
				next_existFleet = SchDutyUtils::existFleetsInDuty(duty);
			}
		}
	}

	if (!before_existFleet && !next_existFleet) {
		//当前Duty和下一个Duty没有货机机型，则不用按计划时间进行检查
		return true;
	}

	if (next_start - before_end < minRest * 3600) {
		...
		_ruleViolation.SetParam("sch_rest", TimeUtils::MinutesTohhmm(static_cast<int>(next_start - before_end) / 60));
...
void CheckSchConsecutiveWOCLRestForEvaRule::ThrowRuleViolation() const {
	RULE_VIOLATION* rv = new RULE_VIOLATION();
	//rv->crewId = this->_dbData->crewList[pCrew->crewIndex]->idCrew;
	string msg = "For the concecutive WOCL duties(" + _ruleViolation.GetParam("number") + "), the schedule rest " + _ruleViolation.GetParam("sch_rest");
...
void CheckSchConsecutiveWOCLRestForEvaRule::ThrowRuleViolationForDutyNumber() const {
	RULE_VIOLATION* rv = new RULE_VIOLATION();
	//rv->crewId = this->_dbData->crewList[pCrew->crewIndex]->idCrew;
...
void CheckSchConsecutiveWOCLRestForEvaRule::ParseParam(const InputType& input) {
	//add by hexd 添加DBRule支持
	for (const auto& dbRule : input.dbRules) {
		_ruleParams.emplace_back(CheckSchConsecutiveWOCLRestForEvaRuleParam(this));
