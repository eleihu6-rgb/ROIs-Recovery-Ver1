/**
 * @file CheckSchMinRestForEvaFdRuleParam.h
 * @brief
 * @author xuedong.he
...
void CheckSchMinRestForEvaFdRuleParam::ParseParam(const std::string &paramString) {
...
void CheckSchMinRestForEvaFdRuleParam::ParseParam(const DBRule& dbRule) {
...
bool CheckSchMinRestForEvaFdRuleParam::MatchComposition(const Duty& duty) const {
...
bool CheckSchMinRestForEvaFdRuleParam::MatchDutyAssignmentGroup(const Duty& duty) const {
...
bool CheckSchMinRestForEvaFdRuleParam::MatchDutyFleetGroups(const Duty& duty) const {
...
bool CheckSchMinRestForEvaFdRuleParam::MatchDutyTypes(const Duty& duty) const {
	if (_dutyTypes.empty()) {
		return true;
	}
	vector<Segment*> segments = duty.getSegments();
...
bool CheckSchMinRestForEvaFdRuleParam::MatchBLHRanges(const Duty& duty) const {
	long blh = const_cast<Duty&>(duty).getScheduleBlockTime();
	return (blh >= _blhRangesMinutesLower && blh <= _blhRangesMinutesUpper);
}

bool CheckSchMinRestForEvaFdRuleParam::MatchFDPRanges(const Duty& duty) const {
	long fdp = const_cast<Duty&>(duty).getScheduleFDP();
	return (fdp >= _fdpRangesMinutesLower && fdp <= _fdpRangesMinutesUpper);
}

bool CheckSchMinRestForEvaFdRuleParam::MatchRule(const Duty& duty) const {
...
bool CheckSchMinRestForEvaFdRuleParam::CheckRule(const Duty* currDuty, const Duty* nextDuty, const int minRest) const {
...
bool CheckSchMinRestForEvaFdRuleParam::CheckRule(const Duty* currDuty, const WorkPeriod* nextWorkPeriod, const int minRest) const {
	...
	int scheduleRest = currDuty->getActualRest();
	time_t schRestStartTimeUtc = SchDutyUtils::GetDutySchEndTimeUtc(currDuty);
	if (nextWorkPeriod == nullptr) {
		scheduleRest = minRest;
	}
	else if (nextWorkPeriod->GetWorkType() == WorkType::FltDuty) {
		Duty* nextDuty = (Duty*)nextWorkPeriod->GetWork();
		scheduleRest = SchDutyUtils::GetScheduleRestMinutes(currDuty, nextDuty, this->GetRule()->GetDataContext());
	}
	else if (nextWorkPeriod->GetWorkType() == WorkType::GroundRoster) {
		ROSTER* nextRoster = (ROSTER*)nextWorkPeriod->GetWork();
		scheduleRest = SchDutyUtils::GetScheduleRestMinutes(currDuty, nextRoster, this->GetRule()->GetDataContext());
	}
