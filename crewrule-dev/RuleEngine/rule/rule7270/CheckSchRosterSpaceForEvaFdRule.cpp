...
*/
//7270
bool LegalityChecker::checkSchRosterSpaceByLableAndAtt_ForEvaFd(RULE_LEGALITY * pCrew, const DBRule* singleRule)
{
	bool isValid = true;
	...
	rule7270* ruleParam = (rule7270*)singleRule->parsedParam.get();
	...
	if (strUnit == "CD")
		lSpace = (atof(strSpace.c_str()) + 1) * 24 * 3600;
	else
	...
	if (g_rule7270_cache.find(key) != g_rule7270_cache.end())
	{
		tempA = g_rule7270_cache[key].first.get();
		tempB = g_rule7270_cache[key].second.get();
		tempA->crewBase = base;
		tempB->crewBase = base;
	}
	else
	{
		g_rule7270_cache[key] = make_pair(shared_ptr<roster_space>(new roster_space), shared_ptr<roster_space>(new roster_space));
		tempA = g_rule7270_cache[key].first.get();
		tempB = g_rule7270_cache[key].second.get();
	...
	*/
	time_t currRoster_existFleet = false, nextRoster_existFleet = false;
	time_t start, end;
	for (std::size_t a = 0; a < rosterA.size(); ++a)
	{
		...
		for (std::size_t b = 0; b < rosterB.size(); ++b)
		{
			...
			currRoster_existFleet = SchDutyUtils::existFleetsInPairing(rosters[iCurrentRoster]->pairing);
			nextRoster_existFleet = SchDutyUtils::existFleetsInPairing(rosters[iNextRoster]->pairing);
			if (!currRoster_existFleet && !nextRoster_existFleet) {
				//当前Roster和下一个Roster没有货机机型，则不用按计划时间进行检查
				continue;
			}

			if (bUtilizePostRest) {
				lEndDT = rosters[iCurrentRoster]->actRestStrUtc;
				if (rosters[iCurrentRoster]->pairing != nullptr) //roster是飞行任务，则使用Pairing获得计划结束时间
					lEndDT = SchDutyUtils::GetPairingSchEndTimeUtc(rosters[iCurrentRoster]->pairing);
			...
			else
			{
				...
				if (rosters[iCurrentRoster]->pairing)
					lEndDT = SchDutyUtils::GetPairingEndTimeIncludingRestUtcSch(rosters[iCurrentRoster]->pairing);
			}
			lStaDT = rosters[iNextRoster]->actStrUtc;
			if (rosters[iNextRoster]->pairing != nullptr) {//roster是飞行任务，则使用Pairing获得计划开始时间
				lStaDT = SchDutyUtils::GetPairingSchStartTimeUtc(rosters[iNextRoster]->pairing);
			}
			start = lEndDT;
			end = lStaDT;

			//20181228 ain, mantis#3680, 7270 unit=CD时时区按crewBase
			...
			if (strUnit == "CD")
			{
				lEndDT = Utility::GetInstancePtr()->getLocalDayStartInUTC(lEndDT, offsetMinutes);
			...
			if (interval < lSpace)
			{
					...
					|| (this->GetApplication() == ROSTER_OPTIMIZER && (rosters[iCurrentRoster]->source!="PA" || rosters[iNextRoster]->source != "PA")))
				{
					...
					string strScheduleSpace;
					if (strUnit == "CD")
					{
						strScheduleSpace = std::to_string((interval) / (24 * 3600)) + " CD";
					}
					else
					{
						strScheduleSpace =  Utility::GetInstancePtr()->formatMinutes((interval < 0 ? interval * -1 : interval) / 60);
					}
					...
					errorMsg += ") and (" + nextRosterLabel + ") is " + (interval < 0 ? ("-" + strScheduleSpace) : strScheduleSpace);
					...
					// 目前Java RuleCombination 7270法規只用到iCurrentRosterLabel, iNextRosterLabel, strSpace, strUnit 4項
					...
					rv->operation_result.insert(pair<string, string>("strScheduleSpace", (interval < 0 ? ("-" + strScheduleSpace) : strScheduleSpace)));
