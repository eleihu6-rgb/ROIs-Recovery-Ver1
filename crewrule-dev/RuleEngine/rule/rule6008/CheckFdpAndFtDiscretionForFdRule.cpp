/**
 * @file CheckFdpAndFtDiscretionForFdRule.cpp
 * @brief
 * @author xuedong.he
...
#include "../RuleSytem.h"
#include "CheckFdpAndFtDiscretionForFdRule.h"
#include "UtilFunc.h"
#include "Utility.h"
...
bool CheckFdpAndFtDiscretionForFdRule::CheckRule(const Pairing* pairing) const {
	return CheckRule(pairing->getDutyVec());
}

bool CheckFdpAndFtDiscretionForFdRule::CheckRule(const Duty* duty) const {
	vector<Duty*> duties(1, const_cast<Duty*>(duty));
	return CheckRule(duties);
}

bool CheckFdpAndFtDiscretionForFdRule::CheckRule(const vector<Duty*>& duties) const {
	if (duties.empty()) {
		return true;
	}
	...
	for (auto& duty : duties) {
		for (const auto & ruleParam : _ruleParams) {
			...
			bool valid = CheckRule(next, duty, duties, ruleParam);
			if (!valid) {
				passAllRule = false;
			}
			if (!next) {
...
bool CheckFdpAndFtDiscretionForFdRule::CheckRule(bool& next, const Duty* currDuty, const vector<Duty*>& duties, const FdpAndFtDiscretionForFdRuleParam& ruleParam) const {
	bool valid = true;
	next = true;
	if (ruleParam.MatchParam(*currDuty, duties)) {
		next = false;

		int warnCode = ruleParam.CheckParam(*currDuty, duties);
		if ((warnCode & (int)FdpAndFtDiscretionForFdRuleParam::WarnCode::FDP_EXTENSION_WARN) == (int)FdpAndFtDiscretionForFdRuleParam::WarnCode::FDP_EXTENSION_WARN) {
			valid = false;
		...
		if ((warnCode & (int)FdpAndFtDiscretionForFdRuleParam::WarnCode::FT_EXTENSION_WARN) == (int)FdpAndFtDiscretionForFdRuleParam::WarnCode::FT_EXTENSION_WARN) {
			valid = false;
		...
		if ((warnCode & (int)FdpAndFtDiscretionForFdRuleParam::WarnCode::DP_EXTENSION_WARN) == (int)FdpAndFtDiscretionForFdRuleParam::WarnCode::DP_EXTENSION_WARN) {
			valid = false;
			if (currDuty->supportDiscretionType(DiscretionType::DP)) {
				_ruleViolation.SetParam("dpMaxExtension", ruleParam._dpMaxExtension);
			}
			else {
				_ruleViolation.SetParam("dpMaxExtension", "0");
			}
...
void CheckFdpAndFtDiscretionForFdRule::ParseParam(const InputType& input) {
...
void CheckFdpAndFtDiscretionForFdRule::ThrowFdpDiscretioRuleViolation(const Duty* duty) const {
	...
	rv->idRule = CheckFdpAndFtDiscretionForFdRule::RuleFuncId;
	...
	if (_ruleViolation.GetRuleLegality() != nullptr) {
		...
		_ruleViolation.SetLegalityMessage(const_cast<Duty*>(duty), msg, CheckFdpAndFtDiscretionForFdRule::RuleFuncId);
	}
...
void CheckFdpAndFtDiscretionForFdRule::ThrowFtDiscretioRuleViolation(const Duty* duty) const {
...
void CheckFdpAndFtDiscretionForFdRule::ThrowDPDiscretioRuleViolation(const Duty* duty) const {
	string currDP = TimeUtils::MinutesTohhmm(const_cast<Duty*>(duty)->getActualDP());
	string maxDP = TimeUtils::MinutesTohhmm(duty->getLimitationValue(RULE_LIMITATION_TYPE::MAX_DP));
	string dpMaxExtension = _ruleViolation.GetParam("dpMaxExtension");
	std::string msg = "Duty Period time ({0:currDP}) is more than the maximum DP ({1:maxDP}) and extension DP ({2:dpMaxExtension}) limitation.";
	msg = StringUtils::Format(msg, currDP, maxDP, dpMaxExtension);
	...
	rv->operation_result.insert(pair<string, string>("ftMaxExtension", dpMaxExtension));
	_ruleViolation.AddRuleViolations(rv);
}