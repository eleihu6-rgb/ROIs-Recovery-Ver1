...
unsigned char MemoryMapped::at(size_t offset) const
{
  // checks
  if (!_mappedView) {
    Logger::getRuleLogger()->error("invalid_argument No view mapped");
	throw std::invalid_argument("No view mapped");
  }
  if (offset >= _filesize) {
    Logger::getRuleLogger()->error("out_of_range View is not large enough");
...
bool MemoryMapped::remap(uint64_t offset, size_t mappedBytes)
{
  ...
  if (mappedBytes == WholeFile)
    mappedBytes = (std::size_t)_filesize;
