...
t64 mktime_64(struct tm *today)
{
	...
	SystemTimeToT64(&st, &t);
#else
	time_t tt = mktime(today);
	t = (t64) tt + today->tm_gmtoff;
#endif
	return t;
}
...
void gmtime_64_s(t64 t, struct tm* m) {
	...
	SYSTEMTIME st;
	T64ToSystemTime(&t, &st);
	m->tm_wday = st.wDayOfWeek;
	m->tm_min = st.wMinute;
	m->tm_sec = st.wSecond;
	m->tm_mon = st.wMonth - 1;
	m->tm_mday = st.wDay;
	m->tm_hour = st.wHour;
	m->tm_year = st.wYear - 1900;
	{
		SYSTEMTIME styear;
		t64 t64Year;
		memset(&styear, 0, sizeof(styear));
		...
		styear.wDay = 1;
		SystemTimeToT64(&styear, &t64Year);
		m->tm_yday = (int)((t - t64Year) / DAY_IN_SECS);
	}
	m->tm_isdst = 0;
	// *******For Windows**********//
#else
	if (NULL == gmtime_r(&t, m)) {
		return;
	}
#endif
}
