-- Align Flight Number Preference (property 116) with the approved standard-answer semantics.
-- Execute with search_path pointing at the target PBS schema.
-- All saved/favorited property 116 data is intentionally removed; legacy payloads are not migrated.

begin;

create temporary table pbs_flight_number_preference_cleanup_property on commit drop as
select id, property_code
from pbs_bid_property
where bid_type = 'Pairing'
  and property_code = 116;

create temporary table pbs_flight_number_preference_cleanup_keys on commit drop as
select distinct g.bid_id, g.bid_type, g.property_group_key
from pbs_bid_group g
where g.bid_type = 'Pairing'
  and exists (
    select 1
    from pbs_flight_number_preference_cleanup_property property
...
create temporary table pbs_flight_number_preference_cleanup_groups on commit drop as
select distinct g.id as group_id, g.bid_id, g.tier_id
from pbs_bid_group g
where exists (
  select 1
  from pbs_flight_number_preference_cleanup_keys target
...
create temporary table pbs_flight_number_preference_cleanup_bids on commit drop as
select distinct bid_id
from pbs_flight_number_preference_cleanup_groups;
...
begin
  ...
  where favorite.property_code = 116
     or favorite.property_id in (select id from pbs_flight_number_preference_cleanup_property);
  ...
  updated_at = now()
  where tier.bid_id in (select bid_id from pbs_flight_number_preference_cleanup_bids);
  ...
  updated_at = now()
  where bid.id in (select bid_id from pbs_flight_number_preference_cleanup_bids);

  delete from pbs_bid bid
  using pbs_flight_number_preference_cleanup_bids target
  ...
  raise notice 'Flight Number Preference cleanup: configured favorites=%, simple favorites=%, generic favorites=%, occurrences=%, conditions=%, groups=%, empty tiers=%, empty bids=%',
...
set property_name = 'Flight Number Preference',
    award_or_avoid = '["award","avoid"]',
    any_or_every = null,
    operator_options = null,
    validation_json = '{"type":"flight-number-preference","label":"Flight Numbers","multi":true,"dateScope":["specific_dates","date_range"]}',
    tooltip = 'Award/Avoid pairings containing a selected actual flight, optionally limited to flight operating dates.',
    is_active = 1,
