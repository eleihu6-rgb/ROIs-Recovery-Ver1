...
CREATE TABLE rule_violation (
  ...
  rule_instance     varchar(20),                   -- rule INSTANCE code, e.g. '001' (8002/001); NULL if not attributable
  scope_key         varchar(40)    NOT NULL DEFAULT '', -- window signature per param row, e.g. '28CD'/'90CD'/'365CD'; '' = single-window rule
  start_dt          timestamptz    NOT NULL,       -- violation window start (partition key)
