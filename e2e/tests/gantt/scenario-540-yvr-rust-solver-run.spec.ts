/**
 * Scenario 540 ("YVR-RUST Seam Connector") — a NEW (non-baseline) end-to-end
 ...
 *          → live-server POST /api/scenario/540/run (type=LegacyRO, inputSource=db)
 *            → engine-server builds ro_input.txt/input.gz from the remote PG
 *               (F8.ro_input_builder, scenario 540 = YVR base / June 2026)
 ...
 *
 *   2. NOTHING HARDCODED:
 *        - crew scope comes from scenario 540's own definition (YVR pilots),
 ...
 *
 * Rust-7008 is the deterministic kick-off proof (UI click → RUNNING).
 * Rust-7009 waits for the real solve to finish and asserts a DONE YVR roster.
 *
 ...
 *     tests/gantt/scenario-540-yyz-rust-solver-run.spec.ts --reporter=list --no-deps
 */
...
const SCENARIO_NAME = 'YVR-FC-Ver1'
...
test.describe('Scenario 540 — YVR Rust-connector solver run via the UI', () => {
  let token = ''
  ...
  test('Rust-7008 — UI "Kick off run" on YVR scenario 540 reaches RUNNING', async ({ page }) => {
    ...
    // mid-solve, accept that and let Rust-7009 wait for it.
    const pre = await fetchStatus(page.request, token)
    if (pre === 'RUNNING') {
      // eslint-disable-next-line no-console
      console.log('[Rust-7008] scenario 540 already RUNNING from a prior kick-off; nothing to start')
    ...
    // 540 is fully configured (rule set, pairing scenario 0, YVR crew+pairing
    ...
    if (await dialog.isVisible().catch(() => false)) {
      await expect(
        ...
        'unexpected pre-run BLOCKER — 540 should be fully configured',
      ).toHaveCount(0)
    ...
    // eslint-disable-next-line no-console
    console.log(`[Rust-7008] scenario ${SCENARIO_ID} status after UI kick-off = ${status}`)
  ...
  test('Rust-7009 — the YVR Rust solve completes (scenario reaches DONE)', async ({ page }) => {
    ...
    // eslint-disable-next-line no-console
    console.log(`[Rust-7009] scenario ${SCENARIO_ID} terminal status = ${status}`)
    expect(status, 'scenario must finish in DONE (not FAILED)').toBe('DONE')
  })

  test('Rust-7010 — open the finished YVR result in the gantt via the real UI', async ({ page }) => {
    ...
    // eslint-disable-next-line no-console
    console.log(`[Rust-7010] gantt-data warm=${warm} — proceeding to the UI open`)
    ...
    // eslint-disable-next-line no-console
    console.log(`[Rust-7010] opened scenario ${SCENARIO_ID} gantt view — crew rows = ${crewCount}`)
