/**
 * Scenario 539 ("YYZ RUST Seam Connector") — a NEW (non-baseline) end-to-end
 ...
 *          → live-server POST /api/scenario/539/run (type=LegacyRO, inputSource=db)
 *            → engine-server builds ro_input.txt/input.gz from the remote PG
 *               (F8.ro_input_builder, scenario 539 = YYZ base / June 2026)
 *            → engine-server fetches the SCENARIO-SCOPED bid package from pbs-server
 *               POST /api/admin/algorithm-export/scenario-package {periodCode, YYZ pilot crewIds}
 ...
 *
 *   2. NOTHING HARDCODED:
 *        - crew scope comes from scenario 539's own definition (YYZ pilots),
 ...
 *
 * Rust-7005 is the deterministic kick-off proof (UI click → RUNNING).
 * Rust-7006 waits for the real solve to finish and asserts a DONE YVR roster.
 *
 ...
 *     tests/gantt/scenario-539-yyz-rust-solver-run.spec.ts --reporter=list --no-deps
 */
...
const SCENARIO_ID = 539
const SCENARIO_NAME = 'YYZ-FC-Ver1'
...
test.describe('Scenario 539 — YYZ Rust-connector solver run via the UI', () => {
  let token = ''
  ...
  test('Rust-7005 — UI "Kick off run" on YVR scenario 539 reaches RUNNING', async ({ page }) => {
    ...
    // mid-solve, accept that and let Rust-7006 wait for it.
    const pre = await fetchStatus(page.request, token)
    if (pre === 'RUNNING') {
      // eslint-disable-next-line no-console
      console.log('[Rust-7005] scenario 539 already RUNNING from a prior kick-off; nothing to start')
    ...
    // 539 is fully configured (rule set, pairing scenario 0, YYZ crew+pairing
    ...
    if (await dialog.isVisible().catch(() => false)) {
      await expect(
        ...
        'unexpected pre-run BLOCKER — 539 should be fully configured',
      ).toHaveCount(0)
    ...
    // eslint-disable-next-line no-console
    console.log(`[Rust-7005] scenario ${SCENARIO_ID} status after UI kick-off = ${status}`)
  ...
  test('Rust-7006 — the YVR Rust solve completes (scenario reaches DONE)', async ({ page }) => {
    ...
    // eslint-disable-next-line no-console
    console.log(`[Rust-7006] scenario ${SCENARIO_ID} terminal status = ${status}`)
    expect(status, 'scenario must finish in DONE (not FAILED)').toBe('DONE')
  })

  test('Rust-7007 — open the finished YYZ result in the gantt via the real UI', async ({ page }) => {
    ...
    // eslint-disable-next-line no-console
    console.log(`[Rust-7007] gantt-data warm=${warm} — proceeding to the UI open`)
    ...
    // eslint-disable-next-line no-console
    console.log(`[Rust-7007] opened scenario ${SCENARIO_ID} gantt view — crew rows = ${crewCount}`)
