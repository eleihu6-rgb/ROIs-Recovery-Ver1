/**
 * Scenario 543 ("Calgary-CC-Ver1") — a NEW (non-baseline) end-to-end optimization
 * run through the REAL gantt UI for a YYC (Calgary) CABIN-crew scenario, validating
 * the full local pipeline:
 *
 *   gantt UI (Kick off run)
 *     → live-server POST /api/scenario/543/run (type=LegacyRO, inputSource=db)
 *       → engine-server builds ro_input.txt/input.gz from the remote PG
 *          (F8.ro_input_builder, scenario 543 = YYC base / June 2026, division=C)
 *       → engine-server fetches the SCENARIO-SCOPED bid package from pbs-server
 *          POST /api/admin/algorithm-export/scenario-package {periodCode, YYC cabin crewIds}
 *       → ./F8/ro_rust.sh → pbs-engine solver (rule_engine.mode=rust,
 *          problem.crew_type=C via RO_CREW_TYPE) → in-process Rust rule connector
 *       → ro_output.txt → output.gz → scenario RUNNING→DONE
 *
 * Cabin note: the solver filters crews by division; engine resolves scenario 543's
 * own division (C) and sets RO_CREW_TYPE=C so the run is NOT filtered to 0 crews.
 *
 * Rust-7014 is the deterministic kick-off proof (UI click → RUNNING).
 * Rust-7015 waits for the real solve to finish and asserts a DONE roster.
 * Rust-7016 opens the finished result in the gantt via the real Open button.
 *
 ...
 *     tests/gantt/scenario-543-yyc-cc-rust-solver-run.spec.ts --reporter=list --no-deps --headed
 */
...
const SCENARIO_ID = 543
const SCENARIO_NAME = 'Calgary-CC-Ver1'
...
test.describe('Scenario 543 — Calgary cabin Rust-connector solver run via the UI', () => {
  let token = ''
  ...
  test('Rust-7014 — UI "Kick off run" on Calgary cabin scenario 543 reaches RUNNING', async ({ page }) => {
    ...
    // mid-solve, accept that and let Rust-7015 wait for it.
    const pre = await fetchStatus(page.request, token)
    if (pre === 'RUNNING') {
      // eslint-disable-next-line no-console
      console.log('[Rust-7014] scenario 543 already RUNNING from a prior kick-off; nothing to start')
    ...
    if (await dialog.isVisible().catch(() => false)) {
      await expect(
        ...
        'unexpected pre-run BLOCKER — 543 should be fully configured',
      ).toHaveCount(0)
    ...
    // eslint-disable-next-line no-console
    console.log(`[Rust-7014] scenario ${SCENARIO_ID} status after UI kick-off = ${status}`)
  ...
  test('Rust-7015 — the Calgary cabin Rust solve completes (scenario reaches DONE)', async ({ page }) => {
    ...
    // eslint-disable-next-line no-console
    console.log(`[Rust-7015] scenario ${SCENARIO_ID} terminal status = ${status}`)
    expect(status, 'scenario must finish in DONE (not FAILED)').toBe('DONE')
  })

  test('Rust-7016 — open the finished Calgary cabin result in the gantt via the real UI', async ({ page }) => {
    ...
    // eslint-disable-next-line no-console
    console.log(`[Rust-7016] gantt-data warm=${warm} — proceeding to the UI open`)
    ...
    // eslint-disable-next-line no-console
    console.log(`[Rust-7016] opened scenario ${SCENARIO_ID} gantt view — crew rows = ${crewCount}`)
