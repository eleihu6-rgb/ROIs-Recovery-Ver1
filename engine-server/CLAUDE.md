...
# 一键打发布包（编译 + 二进制+配置示例+运行时 deploy.sh
# 打成 dist/rois-optimizer-server.tar.gz，无源码）
./build.sh pack
```
...
> 详见 `docs/modules/engine-server/documents/deployment_guide.md`。
...
- [API 接口文档](../docs/modules/engine-server/documents/api_documentation.md) — 端点定义、请求/响应格式、认证示例
- [系统架构](../docs/modules/engine-server/documents/system_architecture.md) — 分层架构、模块关系、分布式部署
- [认证方案](../docs/modules/engine-server/documents/auth_design_proposal.md) — JWT 设计决策、认证流程、已实施状态
- [架构审查报告](../docs/modules/engine-server/documents/architecture_review.md) — 问题清单、修复记录、优化路线图
- [部署指南](../docs/modules/engine-server/documents/deployment_guide.md) — 环境准备、配置、启动、多机部署
- [需求文档](../docs/modules/engine-server/documents/optimize_server_requirements.md) — 功能/非功能需求
- [实现计划](../docs/modules/engine-server/documents/optimize_server_implementation_plan.md) — 10 个任务的开发计划
