...
class TestJWTAuthSuccess:
    ...
    def test_jwt_prefers_user_code_for_task_actor(self, api_client, test_config):
        """JWT 同时包含姓名和编码时，任务执行人应保存 userCode"""
        token = _make_jwt({
            "userCode": "yuan.z",
            "userName": "Yuan Zhang",
            "iss": "admin@pi-solution",
        ...
        task = task_manager.get_task(resp.json()["task_id"])
        assert task is not None
        assert task.user == "yuan.z"
...
class TestJwtIsAdminExtraction:
    """P3: JWT payload 中的 isAdmin 应被提取到 AuthContext（供后续权限判断使用）"""

    def test_admin_jwt_extracts_is_admin(self, test_config):
        from src.api.auth import _try_jwt_auth
        from src.config.config import config_manager

        cfg = config_manager.get_config()
        cfg.auth.jwt.enabled = True
        cfg.auth.jwt.secret = TEST_JWT_SECRET

        token = _make_jwt({"userName": "boss", "iss": "admin@pi-solution", "isAdmin": 1, "exp": int(time.time()) + 3600})
        ctx = _try_jwt_auth(f"Bearer {token}", cfg)
        assert ctx is not None
        assert ctx.auth_method == "jwt"
        assert ctx.is_admin is True

    def test_non_admin_jwt_extracts_is_admin_false(self, test_config):
        from src.api.auth import _try_jwt_auth
        from src.config.config import config_manager
