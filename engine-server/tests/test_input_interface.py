...
class TestROInput:
    ...
    def test_ro_input_fetch_success(self, test_config, airline):
        ...
        try:
            ...
            content = raw.decode('utf-8')
        # 新场景流：从 /api/scenario/export 拉取 ## SECTION CSV 原始表数据
        assert '## CREWS' in content
        assert '## PAIRINGS' in content
        # 请求命中场景导出端点，且 body 携带 scenarioId
        export_reqs = [r for r in test_config['mock_requests'] if r['path'].endswith('/api/scenario/export')]
        assert len(export_reqs) >= 1
        assert export_reqs[-1]['body_text'] == '5432'
...
class TestInputErrorPaths:
    ...
    def test_live_server_unreachable(self, test_config):
        ...
        with pytest.raises(InputFetchError, match="Failed to fetch input.gz"):
            task._fetch_input_data()
