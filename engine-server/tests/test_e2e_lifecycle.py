...
class TestROEndToEnd:
    ...
    def test_ro_full_lifecycle(self, api_client, test_config, airline):
        ...
        # 验证 RO 新场景流端点被调用
        input_reqs = [r for r in test_config['mock_requests'] if r['path'].endswith('/api/scenario/export')]
        output_reqs = [r for r in test_config['mock_requests'] if r['path'].endswith('/api/scenario/result')]
        assert len(input_reqs) >= 1
        assert len(output_reqs) >= 1


class TestResultFileEndpoint:
    """GET /api/optimize/result/{task_id} 返回 output.gz 字节（approach B）"""

    def test_get_result_returns_output_bytes(self, api_client, test_config):
        import gzip
        import os
        import tempfile
        ...
        t = Task("tid-result-1", "F8", "RO", {"scenarioId": 1})
        t.output_file_path = out
        task_manager.tasks["tid-result-1"] = t

        resp = api_client.get("/api/optimize/result/tid-result-1", headers={"X-Airline": "F8"})
        assert resp.status_code == 200
        assert resp.content[:2] == b"\x1f\x8b"  # gzip magic

    def test_get_result_404_when_missing(self, api_client):
        resp = api_client.get("/api/optimize/result/no-such-task", headers={"X-Airline": "F8"})
...
class TestTaskQueries:
    ...
    def test_system_info(self, api_client, test_config):
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data['service'] == 'ROIS Optimizer Server'
    ...
    def test_optimizer_list(self, api_client, test_config):
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert set(data['optimizers']) == {'PO', 'RO', 'TO', 'Rule', 'LegacyRO'}
