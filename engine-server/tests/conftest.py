...
class MockLiveServerHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # ---- RO Input (new scenario flow) ----
        elif path.endswith('/api/scenario/export'):
            self._send_gzip_response(
                "## CREWS\ncrew_id\nF8001\n## PAIRINGS\nid\n5001\n"
            )

        # ---- RO Output (new scenario flow, metadata JSON) ----
        elif path.endswith('/api/scenario/result'):
            self._send_ok_response()
...
def temp_workspace(tmp_path):
    ...
    for d in [workspace, finished, archive, temp, complete]:
        d.mkdir()
...
def mock_optimizers(tmp_path):
    ...
    for airline in ['BR', 'F8']:
        ...
        legacy_ro_path = str(airline_dir / "legacy_ro.sh")
        _create_mock_optimizer_script(legacy_ro_path)
        scripts[f"{airline}/legacy_ro"] = legacy_ro_path
        ...
        return AirlineConfig(
            optimizers=OptimizersConfig(
                ...
                RO=OptimizerTypeConfig(
                    ...
                    url=OptimizerURLConfig(input="/api/scenario/export", output="/api/scenario/result"),
                    server_integration=True,
                ),
                ...
                LegacyRO=OptimizerTypeConfig(
                    name="Legacy Roster Optimizer",
                    linux=OptimizerOSConfig(path=mock_optimizers[f"{airline}/legacy_ro"]),
                    windows=OptimizerOSConfig(path=mock_optimizers[f"{airline}/legacy_ro"]),
                    url=OptimizerURLConfig(input="/api/scenario/export", output="/api/scenario/result"),
                    server_integration=True,
                ),
