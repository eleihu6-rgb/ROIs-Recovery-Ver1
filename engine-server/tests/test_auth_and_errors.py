...
class TestAPIKeyAuth:
    ...
    def test_placeholder_api_key_rejected(self, api_client, test_config):
        """占位符 API Key（${ROIS_API_KEY:your_api_key_here} 未注入的默认值）不能作为合法凭证"""
        from src.config.config import config_manager
        cfg = config_manager.get_config()
        old_key = cfg.auth.api_key.key
        cfg.auth.api_key.key = "your_api_key_here"
        try:
            resp = api_client.get(
                "/api/system/info",
                headers={"X-Airline": "BR", "X-API-Key": "your_api_key_here"},
            )
            assert resp.status_code == 401
        finally:
            cfg.auth.api_key.key = old_key
...
class TestRequestValidation:
    ...
    def test_invalid_optimizer_type(self, api_client, test_config):
        ...
        assert resp.status_code == 400
        assert "Unsupported optimizer type" in resp.json()['detail']
...
# ============================================================
# API 文档暴露测试（docs/redoc/openapi 关闭）
# ============================================================

class TestDocsExposure:
    """Swagger/ReDoc/OpenAPI 在测试配置下必须关闭，未鉴权接口结构不泄露"""

    def test_docs_redoc_openapi_return_404(self, api_client, test_config):
    ...
    def test_app_docs_urls_disabled(self, api_client, test_config):
        """FastAPI 应用未注册文档端点（docs_url/redoc_url/openapi_url 为 None）"""
        assert api_client.app.docs_url is None
        assert api_client.app.redoc_url is None
        assert api_client.app.openapi_url is None
