...
class AuthContext:
    ...
    is_admin: Optional[bool] = None  # JWT 用户是否管理员（live-server JWT isAdmin===1）
...
def _try_jwt_auth(authorization: str, config) -> Optional[AuthContext]:
    ...
    try:
        ...
        is_admin = bool(payload.get("isAdmin") == 1 or payload.get("is_admin") == 1)
...
def _try_api_key_auth(x_api_key: str, x_airline: str, config) -> Optional[AuthContext]:
    ...
    # 占位符/未解析密钥不能作为合法凭证：如 ${ROIS_API_KEY:your_api_key_here} 未注入时
    # 落到默认值 your_api_key_here，任何人猜中即可绕过 JWT。见 ConfigManager._is_unresolved_secret。
    if config_manager._is_unresolved_secret(config.auth.api_key.key):
        return None

    # 先检查航司特定 API Key
    if x_airline and x_airline in config.auth.airline_auth:
        airline_auth = config.auth.airline_auth[x_airline]
        if (airline_auth.api_key
                and not config_manager._is_unresolved_secret(airline_auth.api_key)
                and _safe_compare(x_api_key, airline_auth.api_key)):
...
def _try_bearer_token_auth(authorization: str, x_airline: str, config) -> Optional[AuthContext]:
    ...
    # 占位符/未解析 Bearer Token 不能作为合法凭证（同 API Key 防护）。
    if config_manager._is_unresolved_secret(config.auth.bearer_token.token):
        return None

    # 先检查航司特定 Bearer Token
    if x_airline and x_airline in config.auth.airline_auth:
        airline_auth = config.auth.airline_auth[x_airline]
        if (airline_auth.bearer_token
                and not config_manager._is_unresolved_secret(airline_auth.bearer_token)
                and _safe_compare(token, airline_auth.bearer_token)):
...
def verify_token(
    ...
    if ctx is not None:
        ...
        if not ctx.airline:
            raise HTTPException(
                ...
                detail="JWT authenticated, but the X-Airline header is missing",
            )
    ...
    if not x_airline:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Missing X-Airline header",
        )
