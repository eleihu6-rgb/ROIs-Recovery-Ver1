...
from src.utils.prefixed_redis import PrefixedRedis

logger = logging.getLogger(__name__)

# Redis key 前缀常量（裸 key，不带 env 前缀；PrefixedRedis 会自动加 <env>:）
...
class RedisManager:
    ...
    def _connect(self):
        ...
        try:
            ...
            raw.ping()
            # Wrap the raw client so every key-accepting method transparently
            # prepends <REDIS_KEY_PREFIX>:. See utils/prefixed_redis.py.
            self.redis_client = PrefixedRedis(raw)
