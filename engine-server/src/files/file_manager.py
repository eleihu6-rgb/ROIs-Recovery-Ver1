...
class FileManager:
    ...
    def move_to_complete(
        ...
        """将成功的场景任务目录移动至 complete/<airline>/<scenario_id>/。

        When `version` is provided, the directory becomes
        complete/<airline>/<scenario_id>/<version>/<timestamp>/.
        ...
        with self._lock:
            try:
                ...
                if version:
                    dst_dir = os.path.join(dst_dir, version, timestamp)
    ...
    def _cleanup_expired_tree(self, base_dir: str, retention_days: int, label: str) -> bool:
        """清理目录树中超过保留期的文件，并删除空目录。"""
        try:
            ...
            if cleaned_count > 0:
                logger.info("清理%s过期文件完成，删除 %d 个文件", label, cleaned_count)
            return True
        except Exception as e:
            logger.error("清理%s过期文件失败: %s", label, e, exc_info=True)
            return False

    def cleanup_expired_files(self) -> bool:
        """清理 archive 文件夹内超过指定日期的文件。"""
        with self._lock:
            return self._cleanup_expired_tree(
                self.paths.archive_dir,
                self.file_management.cleanup_days,
                "archive",
            )

    def _compress_complete_entry(self, entry_path: str, entry_name: str) -> bool:
        """将 complete 下的单个目录或文件压缩成 tar.gz。"""
        parent_dir = os.path.dirname(entry_path)
        target_name = f"{entry_name}.tar.gz"
        ...
        try:
            if os.path.exists(target_path):
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                short_id = uuid.uuid4().hex[:6]
                target_path = os.path.join(parent_dir, f"{entry_name}_{timestamp}_{short_id}.tar.gz")

            source_mtime = os.path.getmtime(entry_path)
            with tarfile.open(target_path, "w:gz") as tar:
                tar.add(entry_path, arcname=entry_name)
            ...
            logger.info("complete 条目已压缩为 tar.gz: %s", target_path)
            return True
        except Exception as e:
            logger.warning("complete 条目压缩失败: %s, entry=%s", e, entry_path, exc_info=True)
            return False

    def cleanup_expired_complete_files(self) -> bool:
        """complete 目录保留策略已关闭，保留所有历史版本文件。"""
        with self._lock:
            logger.info("complete retention cleanup disabled; preserving scenario version history")
            return True

    def delete_complete_version(self, airline: str, scenario_id: str, version: str | None = None) -> bool:
        """删除某个场景版本目录或整个场景 complete 目录。"""
        with self._lock:
            try:
                base_dir = os.path.join(self.paths.complete_dir, airline, str(scenario_id))
                target = os.path.join(base_dir, version) if version else base_dir
                ...
                logger.error("删除complete版本失败: %s", e, exc_info=True)
                return False
