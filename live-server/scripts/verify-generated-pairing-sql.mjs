...
try {
  ...
  for (const preflightCase of generatedSqlPreflightCases) {
    ...
    try {
      await client.query(
        `explain select p.id from ${liveSchema}.pairing p where ${condition} limit 1`,
        sqlBuilder.params,
      );
  ...
  console.log(`PASS live-server generated SQL preflight (${generatedSqlPreflightCases.length} cases).`);
} catch (error) {
  try {
    await client.query("rollback");
  } catch {
