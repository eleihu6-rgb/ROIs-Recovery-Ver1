...
export async function rule3007(source, ctx) {
  const instances = ctx.instancesOf(3007)
  if (!instances.length) { ctx.log('3007: no instances in rule set — skipped'); return [] }
  if (typeof source.fdpDuties !== 'function') {
    ctx.log('3007: source does not provide fdpDuties() — skipped')
    return []
  }
  const segs = await source.fdpDuties()
  if (!segs?.length) return []

  const lines = []
  lines.push('B\tINCLUDE CHECK IN\tY')
  lines.push('B\tINCLUDE CHECK OUT\tN')
  for (const inst of ctx.instancesOf(2107)) {
    const H = headerIndexer(inst.header)
    const defI = H('DEFINITION')
    const valI = H('VALUE')
    if (defI < 0 || valI < 0) continue
    for (const row of inst.rows ?? []) {
      const def = String(row[defI] ?? '').trim()
      const val = String(row[valI] ?? '').trim()
      if (def) lines.push(['B', def, val].map(cleanTsv).join('\t'))
    }
  }
  const cio = ctx.instancesOf(3010)[0]
  if (cio) {
    const H = headerIndexer(cio.header)
    const row0 = cio.rows?.[0]
    const brief = fieldRaw(row0, H, 'BRIEF', '60')
    const debrief = fieldRaw(row0, H, 'DEBRIEF', '15')
    lines.push(['C', brief, debrief].join('\t'))
  } else {
    lines.push('C\t60\t15')
  }
  for (const inst of ctx.instancesOf(2102)) {
    const H = headerIndexer(inst.header)
    for (const row of inst.rows ?? []) {
      lines.push([
        'T',
        fieldOrStar(row, H, 'INBOUND'),
        fieldOrStar(row, H, 'OUTBOUND'),
        fieldOrStar(row, H, 'AIRPORT'),
        fieldOrStar(row, H, 'FLEETS'),
        fieldRaw(row, H, 'MAX TURNTIME', '02:00'),
        fieldRaw(row, H, 'PSEUDO BRIEF', '00:20'),
        fieldRaw(row, H, 'PSEUDO DEBRIEF', '00:15'),
        fieldRaw(row, H, 'PSEUDO PICK UP', '00:00'),
        fieldRaw(row, H, 'PSEUDO DROP OFF', '00:00'),
        fieldRaw(row, H, 'IS SPLIT DUTY', 'Y'),
      ].map(cleanTsv).join('\t'))
    }
  }
  lines.push('A\tFLY\t1.0\tFLY')

  const valid = []
  for (const inst of instances) {
    const H = headerIndexer(inst.header)
    if (H('MAX FDP') < 0) {
      ctx.log(`skip 3007/${inst.instance}: MAX FDP column missing`)
      continue
    }
    lines.push(['H', ...(inst.header ?? [])].map(cleanTsv).join('\t'))
    for (const [rowIndex, row] of (inst.rows ?? []).entries()) {
      valid.push({ inst, rowIndex, row, H })
      lines.push(['R', String(rowIndex), ...(row ?? [])].map(cleanTsv).join('\t'))
    }
  }
  if (!valid.length) return []
  ...
  for (const r of segs) {
    const crew = String(r.crew_id ?? '').trim()
    const pairingId = Number(r.pairing_id)
    const dutySeq = Number(r.duty_seq)
    if (!crew || !Number.isFinite(pairingId) || !Number.isFinite(dutySeq)) continue
    const key = `${crew}\t${pairingId}\t${dutySeq}`
    if (!duties.has(key)) {
      duties.set(key, {
        crew, pairingId, dutySeq,
        assignmentGroup: String(r.assignment_group ?? 'FLY').trim() || 'FLY',
        fdpMin: r.duty_sch_fdp_min,
  ...
  for (const [key, duty] of duties) {
    ...
    lines.push([
      'D', key, duty.crew, String(duty.pairingId), String(duty.dutySeq), duty.assignmentGroup, fdpCell,
      '0', '', '',
      first?.brief_start ?? first?.start_act ?? '',
      first?.start_act ?? '',
      first?.brief_start ?? first?.start_act ?? '',
      last?.debrief_end ?? last?.end_act ?? '',
      duty.manual ? 'Y' : 'N',
      String(duty.discretion),
    ].map(cleanTsv).join('\t'))
    for (const s of segsSorted) {
      lines.push([
        'S', key, String(s.flt_id ?? 0), s.seg_assignment || 'FLY',
        s.start_act, s.end_act, s.start_sch, s.end_sch, '0', 'Y', '',
        s.dep_arp ?? '', s.arv_arp ?? '', s.fleet_seg ?? '', '', '*',
      ].map(cleanTsv).join('\t'))
    }
    const node = (type, name, start, end) => {
      if (start == null || end == null || start === '' || end === '') return
      lines.push(['N', key, type, name, String(start), String(end), '0', '0'].join('\t'))
    }
    node('DUTY', 'PICKUP', first?.pickup_start, first?.pickup_end)
    node('DUTY', 'BRIEF', first?.brief_start, first?.brief_end)
    node('DUTY', 'DEBRIEF', last?.debrief_start, last?.debrief_end)
    node('DUTY', 'DROPOFF', last?.dropoff_start, last?.dropoff_end)
  }
  ...
  const persist = []
  const seenF = new Set()
  for (const cols of await binRunner('check-3007', ['--emit-tsv', '--application', 'editor'], lines.join('\n'))) {
    if (cols[0] === 'F' && cols.length >= 5 && cols[5] === 'N') {
      const pairingId = Number(cols[2])
      const dutySeq = Number(cols[3])
      const fdpMin = Number(cols[4])
      const k = `${pairingId}\t${dutySeq}`
      if (!seenF.has(k) && Number.isFinite(pairingId) && Number.isFinite(dutySeq) && Number.isFinite(fdpMin)) {
        seenF.add(k)
        persist.push({ pairing_id: pairingId, duty_seq: dutySeq, fdp_min: fdpMin })
      }
    }
    if (cols[0] !== 'V' || cols.length < 11) continue
    const crewId = String(cols[2]).trim()
    const pairingId = Number(cols[3])
    const dutySeq = Number(cols[4])
    const ruleId = String(cols[5])
    const fdpMin = Number(cols[6])
    ...
    out.push({
      ...
      duty_seq: dutySeq,
      rule_code: '3007',
      rule_instance: matched?.inst?.instance ?? '001',
      scope_key: ruleId,
      start_dt: new Date((Number.isFinite(startUtc) ? startUtc : 0) * 1000).toISOString(),
      end_dt: new Date((Number.isFinite(endUtc) ? endUtc : 0) * 1000).toISOString(),
      severity: 2,
  ...
  if (persist.length && typeof source.persistDutyFdp === 'function') {
    await source.persistDutyFdp(persist)
  }
  return out
}

export const RULES = [rule1001, rule8002, rule8056, rule8071, rule8072, rule7509, rule8030, rule8004, rule7305, rule7505, rule7507, rule7506, rule7501, rule7508, rule7503, rule7504, rule3007]
