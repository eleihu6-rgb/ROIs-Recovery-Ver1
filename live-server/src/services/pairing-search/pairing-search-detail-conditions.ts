...
type AirportPreferenceBid = Extract<PbsPairingBidValue, { type: "airport-preference" }>;
type FlightLegsPerDutyBid = Extract<PbsPairingBidValue, { type: "flight-legs-per-duty" }>;
type RedeyePreferenceBid = Extract<PbsPairingBidValue, { type: "redeye-preference" }>;
type WorkDayPreferenceBid = Extract<PbsPairingBidValue, { type: "work-day-preference" }>;
...
const buildFlightLegsDateClause = (
  ...
  if (bid.dateScope.mode === "specific_dates") {
    if (
      ...
      throw new LineholderBidServiceError(400, `Invalid event dates for ${propertyName}.`);
    }
  ...
  if (
    ...
    throw new LineholderBidServiceError(400, `Invalid event date range for ${propertyName}.`);
  }
...
const buildWorkDayPreferenceCondition = (
  ...
  if (bid.days.length === 0 || bid.days.some((day) => (
    !TIME_OF_DAY_PATTERN.test(day.checkInFrom ?? "")
    || !TIME_OF_DAY_PATTERN.test(day.checkInTo ?? "")
  ...
  const dateClause = !dateScope
    ...
    : dateScope.mode === "specific_dates"
      ? dateScope.dates.length > 0 && dateScope.dates.every((date) => ISO_DATE_PATTERN.test(date))
        ? `work_day_events.event_date = any(${sqlBuilder.addParam(dateScope.dates)}::date[])`
        : null
      : ISO_DATE_PATTERN.test(dateScope.from) && ISO_DATE_PATTERN.test(dateScope.to) && dateScope.from <= dateScope.to
  ...
  if (dateScope && !dateClause) {
    throw new LineholderBidServiceError(400, `Invalid event date scope for ${propertyName}.`);
  }
  ...
  const dayClauses = bid.days.flatMap((day) => {
    ...
    if (day.checkInFrom && day.checkInTo) {
      ...
      const timeClause = day.checkInFrom <= day.checkInTo
  ...
  if (dayClauses.length === 0) {
    throw new LineholderBidServiceError(400, `Missing work days for ${propertyName}.`);
  }
...
const buildUtcTimestampToLocalDateExpression = ({
  timestampExpression,
  zoneExpression,
}: {
  timestampExpression: string;
  zoneExpression: string;
}) => `((${timestampExpression} at time zone 'UTC') at time zone ${zoneExpression})::date`;
...
const buildAirportPreferenceRowsQuery = (
  ...
  const airportCodes = normalizeAirportCodes(
    bid.locations.filter((location) => location.kind === "airport").map((location) => location.code),
  );
  const cityCodes = normalizeAirportCodes(
    bid.locations.filter((location) => location.kind === "city").map((location) => location.code),
  );

  if (airportCodes.length === 0 && cityCodes.length === 0) {
    throw new LineholderBidServiceError(400, `Missing airport or city values for ${propertyName}.`);
  }

  const locationClauses: string[] = [];
  if (airportCodes.length > 0) {
    locationClauses.push(`airport_events.airport_code = any(${sqlBuilder.addParam(airportCodes)}::text[])`);
  }
  if (cityCodes.length > 0) {
    locationClauses.push(`airport_events.city_code = any(${sqlBuilder.addParam(cityCodes)}::text[])`);
  }

  const eventClause = bid.event === "landing"
...
const buildAirportPreferenceDateClause = (
  ...
  if (dateScope.mode === "specific_dates") {
    if (dateScope.dates.length === 0) {
      throw new LineholderBidServiceError(400, `Missing date values for ${propertyName}.`);
    }

    if (!dateScope.dates.every((date) => /^\d{4}-\d{2}-\d{2}$/.test(date))) {
      throw new LineholderBidServiceError(400, `Invalid date value for ${propertyName}.`);
    }
  ...
  if (
    ...
    throw new LineholderBidServiceError(400, `Invalid date range for ${propertyName}.`);
  }
...
const buildAirportPreferenceCondition = (
  ...
  if (property.bid.event === "landing" && property.bid.minimumLayoverDuration) {
    throw new LineholderBidServiceError(400, `Landing Airport Preference cannot include layover duration.`);
  }

  const { eventClause, locationClause, sourceQuery } = buildAirportPreferenceRowsQuery(
    property.bid,
    liveSchema,
    sqlBuilder,
    property.name,
  ...
  const clauses = [
    eventClause,
    locationClause,
    buildAirportPreferenceDateClause(property.bid.dateScope, sqlBuilder, property.name),
  ...
  const filteredQuery = clauses.length > 0
...
const buildRedeyePreferenceDateClause = (
  ...
  if (dateScope.mode === "specific_dates") {
    if (
      ...
      || dateScope.dates.some((date) => !ISO_DATE_PATTERN.test(date))      || dateScope.dates.some((date) => !isDateInPairingSearchPeriod(date, context))
    ) {
      throw new LineholderBidServiceError(400, `Invalid flight dates for ${propertyName}.`);
    }
  ...
  if (
    ...
    || dateScope.to < dateScope.from    || !isDateInPairingSearchPeriod(dateScope.from, context)
...
const buildRedeyePreferenceCondition = (
  ...
  if (!context.redeye?.available) {
    throw new LineholderBidServiceError(503, `Redeye configuration is unavailable for ${propertyName}.`);
  }
  const dateClause = buildRedeyePreferenceDateClause(bid.dateScope, sqlBuilder, propertyName, context);
  const startTime = sqlBuilder.addParam(context.redeye.startTime);
  const endTime = sqlBuilder.addParam(context.redeye.endTime);
  const endOffset = context.redeye.crossesMidnight ? " + interval '1 day'" : "";
  const anchorStartOffset = context.redeye.crossesMidnight ? " - interval '1 day'" : "";
  const clauses = [
    "s.pairing_id = p.id",
    "s.is_deleted = 0",
    // Guard against historical pairing_segment rows with sch_str > sch_end
    // (corrupt ETL/import data). Without this, tstzrange() raises 22000 and
    // aborts the whole EXISTS subquery even for pairings that don't match.
    "s.sch_str_dt_utc <= s.sch_end_dt_utc",
...
export const buildDetailPreviewCondition = (
  ...
  switch (property.propertyCode) {
    case 107: {
      const legsBid = property.bid.type === "flight-legs-per-duty"
        ? property.bid
      ...
      const comparisonClause = legsBid.operator === "Between"
        ? `duty_counts.leg_count between ${sqlBuilder.addParam(legsBid.from)} and ${sqlBuilder.addParam(legsBid.to)}`
        : `duty_counts.leg_count ${legsBid.operator} ${sqlBuilder.addParam(legsBid.legs)}`;
      const dateClause = buildFlightLegsDateClause(legsBid, sqlBuilder, property.name);
      const dutyCountsQuery = `
    ...
    case 110: {
      ...
      return wrapIntent(buildWorkDayPreferenceCondition(property.bid, property.name, liveSchema, sqlBuilder));
    }
    case 123: {
      if (property.bid.type !== "date-or-dow-list" && property.bid.type !== "date-range") {
        break;
      }
    ...
    case 124: {
      ...
      return wrapIntent(buildCompareClause(sqlBuilder, firstDutyLegCountExpression, property.bid));
    }
    case 130: {
      if (property.bid.type !== "stepper") {
        break;
      ...
      return wrapIntent(buildCompareClause(sqlBuilder, lastDutyLegCountExpression, property.bid));
    }
    case 115: {
      ...
      if (values.length === 0) {
        throw new LineholderBidServiceError(400, `Missing crew id values for ${property.name}.`);
      }
    ...
    case 116: {
      ...
      if (values.length === 0) {
        throw new LineholderBidServiceError(400, `Missing flight number values for ${property.name}.`);
      }

      const valuesPlaceholder = sqlBuilder.addParam(values);
      const dateScope = property.bid.dateScope;
      if (dateScope?.mode === "specific_dates" && (
        dateScope.dates.length === 0        || dateScope.dates.some((date) => !ISO_DATE_PATTERN.test(date) || !isDateInPairingSearchPeriod(date, context))
      )) {
        throw new LineholderBidServiceError(400, `Invalid flight dates for ${property.name}.`);
      }
      if (dateScope?.mode === "date_range" && (
        ...
        throw new LineholderBidServiceError(400, `Invalid flight date range for ${property.name}.`);
      }
      const dateClause = dateScope?.mode === "specific_dates"
    ...
    case 117: {
      ...
      return wrapIntent(buildRedeyePreferenceCondition(
        property.bid,
        property.name,
        liveSchema,
        sqlBuilder,
    ...
    case 118: {
      ...
      `;
      const compareClause = buildDurationCompareClause(sqlBuilder, "duty_durations.duty_minutes", property.bid);
      const positiveClause = quantifier === "every"
        ? `
          (
    ...
    case 119: {
      ...
      `;
      const compareClause = buildDurationCompareClause(sqlBuilder, "layover_durations.layover_minutes", property.bid);
      const positiveClause = quantifier === "every"
        ? `
          (
    ...
    case 121: {
      ...
      if (property.bid.operator !== "<" && property.bid.operator !== ">") {
        throw new LineholderBidServiceError(400, `Average Daily Block Time supports < or > only.`);
      }

      const thresholdPlaceholder = sqlBuilder.addParam(parseDurationToMinutes(property.bid.value));
    ...
    case 127: {
      ...
      return wrapIntent(buildDurationCompareClause(sqlBuilder, totalBlockMinutesExpression, property.bid));
    }
    case 125: {
      ...
      if (property.bid.operator !== "<" && property.bid.operator !== ">") {
        throw new LineholderBidServiceError(400, `Credit Per Time Away From Base supports < or > only.`);
      }
      ...
      if (property.bid.unit === "percent") {
        const thresholdPlaceholder = sqlBuilder.addParam(parsePercentToNumber(property.bid.value));
    ...
    case 122: {
      ...
      const datePredicate = dateScope?.mode === "specific_dates"
    ...
    case 101: {
      ...
      const values = property.bid.type === "tag-list"
        ? normalizeAirportCodes(property.bid.values)
        : normalizeAirportCodes([property.bid.value]);

      if (values.length === 0) {
        throw new LineholderBidServiceError(400, `Missing airport values for ${property.name}.`);
      }
    ...
    case 165: {
      ...
      if (values.length === 0) {
        throw new LineholderBidServiceError(400, `Missing station values for ${property.name}.`);
      }
    ...
    case 168: {
      ...
      return wrapIntent(buildAirportPreferenceCondition(
    ...
    case 160: {
      ...
      if (patterns.length === 0) {
        throw new LineholderBidServiceError(400, `Missing aircraft values for ${property.name}.`);
      }

      const likeConditions = patterns.map((pattern) => `upper(coalesce(s.fleet_seg, p.fleet)) like ${sqlBuilder.addParam(`%${pattern}%`)}`);
      return wrapIntent(`
        exists (
          select 1
          from ${liveSchema}.pairing_segment s
