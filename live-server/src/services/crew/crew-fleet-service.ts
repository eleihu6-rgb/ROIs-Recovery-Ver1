...
const CACHE_PREFIX = 'crew:fleet'
const CACHE_TTL = 14400

const currentEff = and(
  lte(crewFleet.effDt, sql`now()`),
  or(isNull(crewFleet.expDt), gt(crewFleet.expDt, sql`now()`)),
)

export const crewFleetService = {
  async list(fastify: FastifyInstance, crewId: string) {
    ...
    return getOrSet(fastify.redis, key, CACHE_TTL, () =>
      fastify.db
        .select()
        .from(crewFleet)
        .where(eq(crewFleet.crewId, crewId))
  ...
  async getCurrent(fastify: FastifyInstance, crewId: string) {
    ...
    return getOrSet(fastify.redis, key, CACHE_TTL, async () => {
      const rows = await fastify.db
        .select()
        .from(crewFleet)
        .where(and(eq(crewFleet.crewId, crewId), currentEff))
        .orderBy(desc(crewFleet.effDt))
        .limit(1)
      return rows[0] ?? null
    })
  ...
  async create(
    ...
    crewId: string,
    data: typeof crewFleet.$inferInsert,
    username: string,
  ) {
  ...
  async update(
    ...
    crewId: string,
    id: number,
    data: Partial<typeof crewFleet.$inferInsert>,
    username: string,
  ) {
    const rows = await fastify.db
      .update(crewFleet)
      .set({ ...data, ...auditUpdate(username) })
      .where(and(eq(crewFleet.id, id), eq(crewFleet.crewId, crewId)))
      .returning()
